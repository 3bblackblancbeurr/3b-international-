...
$PackageDir = Join-Path $SavedRoot "Packages"

New-Item -ItemType Directory -Force -Path $EvidenceDir,$LogDir,$PackageDir | Out-Null
...
function Invoke-Logged {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "$Label failed with exit code $LASTEXITCODE. Log: $LogPath"
    }
    return $LogPath
}
...
function Assert-EvidencePassList {
    ...
    if ($Missing.Count -gt 0) {
        throw "$Label refuse: preuves PASS manquantes: $($Missing -join ', ')."
    }
}
...
$IsSourceCapable = $ForceSourceTargets -or -not (Test-Path $InstalledBuildMarker)

Write-Host "[3B France] EngineRoot=$EngineRoot"
Write-Host "[3B France] Project=$Project"
Write-Host "[3B France] Mode=$Mode"
Write-Host "[3B France] SourceTargets=$IsSourceCapable"
...
try {
    ...
    if ($IsSourceCapable) {
        ...
        Remove-Item (Join-Path $EvidenceDir "compile_partial.json") -ErrorAction SilentlyContinue
    } else {
        $Partial = [ordered]@{
            schema_version = "1.0.0"
            status = "PREPARED"
            engine = "5.8"
            generated_at_utc = [DateTime]::UtcNow.ToString("o")
            targets = @("ThreeBWorldEditor")
            missing_targets = @("ThreeBWorldClient","ThreeBWorldServer")
            reason = "Installed Engine build detected; UE source/server-capable build required for Client and Server target validation."
            logs = $CompileLogs
        }
        ...
        Write-Host "[3B France] Client/Server restent bloques tant qu'un source build UE5.8 n'est pas utilise."
    }
} catch {
...
if ($Mode -in @("WorldPartition","Full")) {
    Assert-EvidencePassList -Label "HLOD/Nav final" -Names @(
...
if ($Mode -eq "Full") {
    if (-not $IsSourceCapable) {
        throw "Packaging Client/Server refuse: un source build UE5.8 est requis."
    }
    if (-not (Test-Path $RunUAT)) {
        throw "RunUAT introuvable: $RunUAT"
    }

    Assert-EvidencePassList -Label "Packaging final" -Names @(
        "hlod.json",
        "navigation.json",
        "pie_singleplayer.json",
        "pie_multiplayer.json",
        "profiling.json"
    )

    $ClientArchive = Join-Path $PackageDir "Client"
    $ServerArchive = Join-Path $PackageDir "Server"
    New-Item -ItemType Directory -Force -Path $ClientArchive,$ServerArchive | Out-Null

    try {
        $ClientPackageLog = Invoke-Logged -Label "Package ThreeBWorld Client" -Exe $RunUAT -Arguments @(
            "BuildCookRun",
            "-project=$Project",
            "-noP4",
            "-platform=Win64",
            ...
            "-unattended",
            "-utf8output"
        ) -LogName "package-client.log"
        Write-Evidence -Name "package_client.json" -Status "PASS" -Kind "packaged_build_smoke_test" -Details @{ log = $ClientPackageLog; archive = $ClientArchive }
    } catch {
        Write-Evidence -Name "package_client.json" -Status "FAIL" -Kind "packaged_build_smoke_test" -Details @{ error = $_.Exception.Message }
        throw
    }

    try {
        $ServerPackageLog = Invoke-Logged -Label "Package ThreeBWorld Server" -Exe $RunUAT -Arguments @(
            "BuildCookRun",
            "-project=$Project",
            "-noP4",
            "-server",
            ...
            "-unattended",
            "-utf8output"
        ) -LogName "package-server.log"
        Write-Evidence -Name "package_server.json" -Status "PASS" -Kind "packaged_build_smoke_test" -Details @{ log = $ServerPackageLog; archive = $ServerArchive }
    } catch {
        Write-Evidence -Name "package_server.json" -Status "FAIL" -Kind "packaged_build_smoke_test" -Details @{ error = $_.Exception.Message }
        throw
    }
}

$GateLog = Join-Path $LogDir "goldmaster-gate.log"
$GateArgs = @($Project,$Map,"-ExecutePythonScript=$GateScript","-Unattended","-NoSplash","-NoP4")
Write-Host ""
Write-Host "=== France Gold Master strict gate ==="
& $EditorCmd @GateArgs 2>&1 | Tee-Object -FilePath $GateLog

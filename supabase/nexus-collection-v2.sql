-- Permanent collectible exposure inside Nexus V2.
-- Ownership, city revision and placement are checked server-side.

create or replace function public.nexus_display_collectible(p_item_instance_id uuid,p_x integer,p_z integer,p_rotation smallint,p_expected_revision bigint)
...
revoke all on function public.nexus_display_collectible(uuid,integer,integer,smallint,bigint) from public;
grant execute on function public.nexus_display_collectible(uuid,integer,integer,smallint,bigint) to authenticated;

create or replace function public.nexus_remove_collectible_display(p_item_instance_id uuid,p_expected_revision bigint)
returns jsonb language plpgsql security definer set search_path=public as $$
...
revoke all on function public.nexus_remove_collectible_display(uuid,bigint) from public;
grant execute on function public.nexus_remove_collectible_display(uuid,bigint) to authenticated;

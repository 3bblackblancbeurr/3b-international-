...
declare v_uid uuid:=auth.uid();v_account public.economy_accounts%rowtype;v_city public.nexus_cities%rowtype;v_place public.nexus_city_placements%rowtype;v_build public.nexus_city_buildings%rowtype;v_price bigint;v_level integer;v_required integer;
begin
 ...
 insert into public.nexus_city_journal(city_id,user_id,action,reference_id,coins,metadata) values(v_city.city_id,v_uid,'build_upgrade',p_placement_id,-v_price,jsonb_build_object('upgrade_level',v_place.upgrade_level,'request_id',p_request_id,'required_level',v_required,'token',0));
 ...
 return jsonb_build_object('ok',true,'idempotent',false,'upgrade_level',v_place.upgrade_level,'coins',v_account.coins,'xp',v_account.xp,'level',v_level,'required_level',v_required,'token',0,'revision',v_city.revision);
end$$;

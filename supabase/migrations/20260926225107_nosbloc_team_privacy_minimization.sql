-- Nosbloc team privacy: collaborators only receive the minimum team fields they need.
      ...
      select jsonb_agg(jsonb_build_object(
        ...
        'members',coalesce((
          select jsonb_agg(jsonb_build_object(
            ...
            'userId',case when p.owner_id=v_uid or m.user_id=v_uid then m.user_id else null end,
            'name',m.display_name,
            'role',m.role_name,
            'shareBps',m.share_bps,
            'status',m.membership_status,
            'invitedAt',case when p.owner_id=v_uid or m.user_id=v_uid then m.invited_at else null end,
            'acceptedAt',m.accepted_at,
            'invitationId',case when p.owner_id=v_uid then (
...
comment on function public.nosbloc_snapshot_api() is
  'Authenticated Nosbloc snapshot. Team UUIDs/invitation IDs are minimized according to project ownership.';

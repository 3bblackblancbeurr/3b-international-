...
declare v_uid uuid:=auth.uid();v_city public.nexus_cities%rowtype;v_account public.economy_accounts%rowtype;v_placements jsonb;v_items jsonb;v_displays jsonb;v_item_count bigint;
begin
 ...
 select coalesce(jsonb_agg(q.obj order by q.acquired_at desc),'[]'::jsonb) into v_items from (
  ...
  where i.owner_id=v_uid and d.active=true order by i.acquired_at desc limit 500
 ) q;

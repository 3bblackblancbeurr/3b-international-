...
begin
 ...
 if exists(select 1 from public.threeb_wallet_ledger where user_id=v_uid and event_key='nexus_build_upgrade' and event_id=p_request_id::text) then return jsonb_build_object('ok',true,'idempotent',true,'coins',v_account.coins,'xp',v_account.xp,'level',least(150,greatest(1,floor(sqrt(greatest(v_account.xp,0)::numeric/110))+1)),'token',0,'revision',v_city.revision);end if;
 ...
 v_level:=least(150,greatest(1,floor(sqrt(greatest(v_account.xp,0)::numeric/110))+1));v_required:=least(150,v_build.unlock_level+v_place.upgrade_level*3);
 ...
 insert into public.nexus_city_journal(city_id,user_id,action,reference_id,coins,metadata) values(v_city.city_id,v_uid,'build_upgrade',p_placement_id,-v_price,jsonb_build_object('upgrade_level',v_place.upgrade_level,'xp_reward',v_xp_reward,'required_level',v_required,'request_id',p_request_id,'token',0));
 update public.nexus_cities set revision=revision+1,city_xp=city_xp+v_xp_reward,city_level=least(50,1+floor(sqrt(greatest(city_xp+v_xp_reward,0)::numeric/140))),updated_at=now() where user_id=v_uid returning revision into v_city.revision;
 return jsonb_build_object('ok',true,'idempotent',false,'upgrade_level',v_place.upgrade_level,'coins',v_account.coins,'xp',v_account.xp,'level',least(150,greatest(1,floor(sqrt(greatest(v_account.xp,0)::numeric/110))+1)),'xp_reward',v_xp_reward,'token',0,'revision',v_city.revision);
end$$;

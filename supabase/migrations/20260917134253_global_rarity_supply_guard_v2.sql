create or replace function public.threeb_enforce_global_item_rarity_supply() returns trigger language plpgsql security definer set search_path=public as $$
...
create or replace function public.nexus_enforce_collectible_supply() returns trigger language plpgsql security definer set search_path='' as $$
...
create or replace function public.threeb_rarity_supply_status() returns table(rarity text,supply_cap integer,minted bigint,remaining bigint) language sql stable security definer set search_path=public as $$

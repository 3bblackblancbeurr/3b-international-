-- Secret 3B — campaign prize_count is the single source of truth for winner cap.
...
revoke all on function public.secret3b_claim_phone_answer(text,text,text,text,text,text)
  from public, anon, authenticated;
grant execute on function public.secret3b_claim_phone_answer(text,text,text,text,text,text)
  to service_role;

-- Keep the legacy config mirror aligned for operational readability, although
-- the atomic award function above no longer trusts it for the winner cap.

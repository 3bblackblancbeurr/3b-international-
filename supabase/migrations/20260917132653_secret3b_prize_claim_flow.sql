-- Secret 3B — secure in-app prize claim flow.
-- The winning code itself is never stored: only its HMAC hash exists in winners.

alter table public.secret3b_phone_winners
...
begin
  if not exists (
    ...
    where conname = 'secret3b_phone_winners_claimed_by_fkey'
      and conrelid = 'public.secret3b_phone_winners'::regclass
  ) then
    alter table public.secret3b_phone_winners
      add constraint secret3b_phone_winners_claimed_by_fkey
      foreign key (claimed_by) references auth.users(id) on delete set null;
  end if;
end $$;

create unique index if not exists secret3b_phone_winners_claimed_by_unique
  on public.secret3b_phone_winners(campaign_id, claimed_by)
  where claimed_by is not null;

create table if not exists public.secret3b_claim_rate_limits (
...
alter table public.secret3b_claim_rate_limits enable row level security;
revoke all on table public.secret3b_claim_rate_limits from public, anon, authenticated;
grant select, insert, update, delete on table public.secret3b_claim_rate_limits to service_role;

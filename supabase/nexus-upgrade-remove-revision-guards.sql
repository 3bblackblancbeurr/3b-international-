-- Applied to Supabase production on 2026-09-17.
-- Prevents stale multi-device upgrade/removal writes from mutating a newer city snapshot.
...
begin
  ...
  if exists(select 1 from public.threeb_wallet_ledger where user_id=v_uid and event_key='nexus_build_upgrade' and event_id=p_request_id::text) then
    return v_wallet||jsonb_build_object('ok',true,'idempotent',true,'revision',v_city.revision);
  end if;
  select * into v_place from public.nexus_city_placements where id=p_placement_id and city_id=v_city.city_id for update;
  ...
  select * into v_build from public.nexus_city_buildings where code=v_place.building_code and active=true;
  ...
  v_wallet:=public.threeb_wallet_apply_server(v_uid,v_xp_reward,-v_price);
...
grant execute on function public.nexus_upgrade_building(uuid,bigint,uuid) to authenticated;
...
begin
  ...
  if not found then raise exception 'nexus_city_required'; end if;
  if v_city.revision<>p_expected_revision then raise exception 'revision_conflict'; end if;
  if exists(select 1 from public.threeb_wallet_ledger where user_id=v_uid and event_key='nexus_build_remove' and event_id=p_request_id::text) then
    return v_wallet||jsonb_build_object('ok',true,'idempotent',true,'revision',v_city.revision);
  end if;
  select * into v_place from public.nexus_city_placements where id=p_placement_id and city_id=v_city.city_id for update;
  if not found then raise exception 'placement_not_found'; end if;
...
grant execute on function public.nexus_remove_building(uuid,bigint,uuid) to authenticated;

...
-- The one-in-a-billion table is stored as exact integer weights.
-- Claims are server-only: clients cannot insert or call the award RPC directly.
...
insert into public.nexus_rarity_config(rarity,weight_per_billion,supply_cap) values
  ('common',700000000,null),('rare',200000000,null),('epic',70000000,null),
  ('special',20000000,null),('ultra-rare',8000000,null),('legendary',1900000,null),
  ('ultimate',99999,8),('unique',1,1)
...
declare cap integer; next_copy integer;
begin
  select coalesce(c.supply_cap,r.supply_cap) into cap
  ...
  if cap is null then new.copy_no:=null; return new; end if;
...
create trigger nexus_collectible_supply_guard before insert on public.nexus_collectible_claims
...
begin
  ...
  return query insert into public.nexus_collectible_claims(item_id,user_id,source_key)
  ...
  if not found then
    return query select c.claim_id,c.item_id,c.user_id,c.copy_no,c.claimed_at from public.nexus_collectible_claims c
      where c.user_id=p_user_id and c.source_key=left(p_source_key,160) limit 1;
  end if;
end;
$$;
...
insert into public.nexus_collectible_catalog(item_id,name,family,rarity,country,supply_cap) values
 ('maison-3b','Maison 3B','building','common','3b',null),('atelier-3b','Atelier 3B','building','common','3b',null),
 ('jardin-3b','Jardin d’union','building','common','3b',null),('tour-matrix','Tour Matrix','building','rare','3b',null),
 ('gare-horizon','Gare Horizon','building','epic','3b',null),('hall-heritage','Hall de l’Héritage','building','special','3b',null),
 ('citadelle-bleue','Citadelle bleue','building','ultra-rare','3b',null),('spire-champagne','Spire champagne','building','legendary','3b',null),
 ('nexus-ultime','Nexus Ultime','building','ultimate','3b',8),('monolithe-unique','Monolithe Unique 3B','building','unique','3b',1)

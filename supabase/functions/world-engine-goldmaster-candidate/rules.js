...
export function teamStats(save){
 ...
 const home=frontierState(save),mastery=Math.min(12,[save.leader,...save.team].reduce((sum,id)=>sum+masteryLevel(save.adventure.mastery?.[id]),0)),order=save.adventure.companionOrder||'follow',homeland=cards.filter(c=>c.country===save.region).length;
 return {health:100+home.camp*8+(TRAVEL_GEAR[save.adventure.avatar?.travelGear]?.health||0)+(path==='nature'?14:path==='tempete'?-8:0)+roles.filter(r=>r==='protecteur').length*12+(load.terrain?10:0)+(load.pierre?10:0)+(order==='guard'?12:0),attack:Math.round(home.forge+mastery+(path==='tempete'?3:path==='ombre'?1:0)+(cardById[save.leader]?.attack||15)+roles.filter(r=>r==='assaillant').length*4+Math.min(8,levelFor(save.xp)-1)+(load.ambiance?3:0)+(load.fragment?3:0)+Math.min(3,homeland)),heal:(path==='lumiere'?3:0)+roles.filter(r=>r==='soigneur').length*4+(order==='support'?3:0),speed:(roles.includes('éclaireur')?1.08:1)+(path==='ombre'?.05:0)+(TRAVEL_GEAR[save.adventure.avatar?.travelGear]?.speed||0)+(order==='scout'?.06:0),window:roles.includes('mystique')?.19:.14,affinity:cards.reduce((n,c)=>n+Math.min(3,Math.floor((save.collection[c.id]-1)/3)),0),traps:load.traps.length,support:!!load.support,energy:!!load.energy};
}
...
export function worldItems(region,save){
 ...
 const c=countryById[region],cards=encounterCards(region,save),guardian=CARDS.find(card=>card.country===region&&card.category==='Carte unique'),valueRule=GUARDIAN_VALUES[region],valueDone=!!save.adventure.values?.[region]?.completed,state=chapterState(save,region);
 return [{id:'hub',type:'portal',name:'Place des huit portes',x:0,z:20,color:'#e9d59e',range:6},{id:region+':story',type:'story',name:CHAPTERS[region].resident.split(',')[0]+' · '+CHAPTERS[region].title,x:11,z:-4,color:c.color,range:6,done:state.restored===3},
 ...
 {id:region+':value',type:'valueTrial',name:valueDone?'Valeur maîtrisée · '+valueRule.value:state.restored<2?'Épreuve scellée · '+valueRule.value:'Épreuve · '+valueRule.value,x:-13,z:-50,color:c.color,range:5,done:valueDone,locked:state.restored<2},...(valueDone||save.seals.includes(region)?[{id:region+':guardian',type:'guardian',name:save.seals.includes(region)?'Défier à nouveau '+guardian.name:guardian.name,detail:guardian.power,card:guardian.id,x:0,z:-57,color:c.color,range:7}]:[])];
}

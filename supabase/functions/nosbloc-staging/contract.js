...
export function normalizeProjectPayload(source = {}) {
  ...
  return {
    ...
    rights: {contentOwned:source.rights?.contentOwned===true,thirdPartyLicensed:source.rights?.thirdPartyLicensed===true,audienceReviewed:source.rights?.audienceReviewed===true},
    splits,
...
export function validateProjectPayload(project) {
  const payload=normalizeProjectPayload(project),totalBps=payload.splits.reduce((sum,row)=>sum+row.shareBps,0),errors=[];
...
export function normalizeVersionStage(value){const stage=String(value||"");if(!STAGES.has(stage))throw new Error("Étape de version invalide.");return stage;}
export function normalizeModerationDecision(value){const decision=String(value||"");if(!DECISIONS.has(decision))throw new Error("Décision invalide.");return decision;}
function stable(value){if(Array.isArray(value))return value.map(stable);if(value&&typeof value==="object")return Object.keys(value).sort().reduce((out,key)=>({...out,[key]:stable(value[key])}),{});return value===undefined?null:value;}

...
const cards=CARDS.map(card=>{
 const path='public/world/card-models/'+card.id+(card.id==='C165'?'-v2':'')+'.glb',b=fs.readFileSync(path),j=JSON.parse(b.subarray(20,20+b.readUInt32LE(12)).toString()),design=CARD_DESIGNS[card.id];
 ...
 if(['person','guardian'].includes(design.kind)&&!j.skins?.length&&!(card.id==='C165'&&j.nodes.some(n=>n.name==='CoqSupreme')&&j.animations.length===7))throw Error('Missing skin: '+card.id);
 if(design.kind==='person'&&!j.materials.some(m=>/ClothColor_ClothColor/.test(m.name)))throw Error('Merged clothing material: '+card.id);
 return {id:card.id,name:card.name,country:card.country,category:card.category,kind:design.kind,url:'/world/card-models/'+card.id+(card.id==='C165'?'-v2':'')+'.glb',bytes:b.length,sha256:crypto.createHash('sha256').update(b).digest('hex'),animations,meshes:j.meshes.length};
});

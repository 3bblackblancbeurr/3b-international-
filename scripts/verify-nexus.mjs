...
async function screenshot(page,name){await page.screenshot({path:`${output}/${name}.png`,fullPage:false});report.screenshots.push(name);}
async function open(page){await page.goto('http://127.0.0.1:4177/tests/nexus-fixture.html');await page.locator('#open').click();await page.locator('dialog[open]').waitFor();}
...
try{
  ...
  const page=await desktop.newPage();listen(page);await open(page);
  await page.locator('.nexus-experience[data-phase="tunnel"]').waitFor({timeout:10000});await webgl(page);await screenshot(page,'desktop-tunnel');await skip(page);
  assert.equal(await page.locator('.nexus-door-choice').count(),8);
  assert.equal(await page.locator('dialog').evaluate(d=>d.parentElement===document.body),true);
  ...
  await page.getByRole('button',{name:'ORIGINE 09'}).click();assert.equal(await page.locator('.nexus-enter-world').isDisabled(),true);await screenshot(page,'desktop-origine');
  ...
  report.checks.push('Country handoff and leaving the Nexus work');
  ...
  await phone.getByRole('button',{name:'ORIGINE 09'}).click();await screenshot(phone,'mobile-origine');assert.equal(await phone.locator('.nexus-enter-world').isDisabled(),true);
  await phone.getByRole('button',{name:'Fermer le Nexus et revenir au passeport'}).click();
  report.checks.push('390×844 touch viewport: horizontal country rail, no document overflow, three closeups and close button');await mobile.close();
  ...
  assert.deepEqual(report.errors,[],'Unexpected browser or shader errors');report.success=true;
}catch(error){report.success=false;report.failure=error.stack;process.exitCode=1;}finally{

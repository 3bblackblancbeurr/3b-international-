...
try{
 for(const mobile of [false,true]){
  const label=mobile?'touch-landscape':'desktop',errors=[];
  ...
  try{
   await page.goto(base+'/#monde-3b',{waitUntil:'domcontentloaded'});
   const editor=page.getByRole('dialog',{name:'Ton personnage',exact:true});
   await editor.waitFor({timeout:120000});
   await page.getByLabel('Nom du personnage',{exact:true}).fill('QA '+label);
   await page.getByRole('button',{name:'Commencer mon voyage',exact:true}).click();
   await page.locator('.avatar-cinematic').waitFor({timeout:60000});
   await page.locator('.avatar-cinematic').getByRole('button',{name:'Passer',exact:true}).click();
   await page.locator('.world-cinematic-director').waitFor({timeout:30000});
   ...
   report.push({device:label,ok:true,checks:['current-entry','avatar-save','cinematic-skip','opening-handoff','pause','reload']});
  }catch(error){
   await page.screenshot({path:out+'/'+label+'-failure.png'}).catch(()=>{});
   fs.writeFileSync(out+'/'+label+'-failure.txt',await page.locator('body').innerText().catch(()=>''));
   report.push({device:label,ok:false,error:error.message,pageErrors:errors});

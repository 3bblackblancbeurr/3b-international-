...
try{
 ...
 for(const [name,width,height] of [['desktop',1440,960],['mobile',390,844],['small-mobile',320,640],['landscape',844,390]]){
  ...
  await page.goto('http://127.0.0.1:4173/tests/fixtures/nexus.html');await page.getByRole('button',{name:'Ouvrir mon passeport'}).click();
  await page.getByRole('button',{name:'Accéder au Nexus'}).click();await page.waitForSelector('.nx-shell[data-nexus-ready="true"]',{timeout:30000});
  ...
  for(const label of ['France — Justice','Algérie — Loyauté','Espagne — Passion','Maroc — Noblesse','Italie — Espoir','Tunisie — Courage','Turquie — Foi','Estonie — Sagesse']){
    await page.getByRole('button',{name:label,exact:true}).click();assert.equal(await page.getByRole('button',{name:label,exact:true}).getAttribute('aria-pressed'),'true');
  }
  ...
  await page.getByRole('button',{name:'Découvrir la porte secrète ORIGINE, verrouillée'}).click();assert.equal(await page.getByRole('button',{name:'Réunir les huit clés'}).isDisabled(),true);
  ...
  await page.getByRole('button',{name:'Ouvrir mon passeport'}).click();await page.getByRole('button',{name:'Accéder au Nexus'}).click();await page.getByRole('button',{name:'Algérie — Loyauté',exact:true}).click();await page.getByRole('button',{name:'Franchir la porte'}).click();
 ...
 const calm=await browser.newContext({viewport:{width:390,height:844},reducedMotion:'reduce'});const cp=await calm.newPage();await cp.goto('http://127.0.0.1:4173/tests/fixtures/nexus.html?calm=1');await cp.getByRole('button',{name:'Ouvrir mon passeport'}).click();await cp.waitForSelector('.nx-shell[data-phase="nexus"]');assert.equal(await cp.locator('.nx-transition').count(),0);await cp.screenshot({path:`${output}/reduced-motion.png`});results.push({name:'reduced-motion',status:'passed'});await calm.close();
 ...
 const fp=await fallback.newPage();await fp.goto('http://127.0.0.1:4173/tests/fixtures/nexus.html?calm=1');await fp.getByRole('button',{name:'Ouvrir mon passeport'}).click();await fp.waitForSelector('.nx-fallback');await fp.getByRole('button',{name:'Italie — Espoir',exact:true}).click();assert.equal(await fp.getByRole('button',{name:'Franchir la porte'}).isEnabled(),true);await fp.screenshot({path:`${output}/no-webgl-fallback.png`});results.push({name:'no-webgl-fallback',status:'passed'});await fallback.close();

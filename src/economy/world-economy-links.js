...
export function constructionPrice(id,upgrade=1){const b=MEGA_BY_ID[id];if(!b)return null;const level=Math.max(1,Math.min(5,Math.floor(upgrade)));const rarityMultiplier={common:1,rare:1.2,epic:1.55,legendary:2.2}[b.rarity]||1;return Math.round(b.cost*rarityMultiplier*Math.pow(1.55,level-1));}

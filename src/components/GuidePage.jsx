...
export default function GuidePage({goTo,menuItems}){
 ...
 <div className="guide-distinction"><article><span>01 / EXPÉRIENCE</span><h2>Les XP font évoluer ta collection.</h2><p>Ils déterminent ton niveau. Tu ne les dépenses pas.</p><strong>{fmt(account.profile?.xp||0)} XP</strong></article><article><span>02 / FIDÉLITÉ</span><h2>Les points font évoluer ta carte boutique.</h2><p>Ils se cumulent et ne se dépensent pas. Les achats remboursés ajustent les gains.</p><strong>{fmt(account.profile?.points||0)} points</strong></article></div>
 ...
 <h2>À quoi sert chaque rubrique ?</h2><div className="guide-directory">{menuItems.filter(i=>i.id!=='guide').map(i=>i.locked?<CompactCard as="article" key={i.id} aria-disabled="true" locked action="Verrouillé" icon={<SectionIcon page={i.id}/>} title={i.label} description={i.description}/>:<CompactCard as={RouteLink} key={i.id} page={i.id} goTo={goTo} icon={<SectionIcon page={i.id}/>} title={i.label} description={i.description}/>)}</div>

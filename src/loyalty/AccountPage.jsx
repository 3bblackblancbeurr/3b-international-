...
export default function AccountPage({legacy,options,toggleOption,goTo}){
 ...
 const[mode,setMode]=useState(()=>{try{return sessionStorage.getItem('3b-auth-intent')==='register'?'register':'login';}catch{return 'login';}}),[fields,setFields]=useState({handle:'',password:'',name:legacy?.name||'',country:legacy?.originCountry||'France',recovery:''}),[error,setError]=useState(''),[busy,setBusy]=useState(false),[prestigeBusy,setPrestigeBusy]=useState(false),[recovery,setRecovery]=useState('');
 ...
 const unlockPrestige=async()=>{
  ...
  try{
   await memberRequest('prestige',{},account.user?.id);
 ...
 return <section className="loyalty-page account-page"><header className="loyalty-intro"><span className="loyalty-eyebrow">3B INTERNATIONAL / MON COMPTE</span><button onClick={()=>goTo('loyalty')}>Mes cartes <ArrowUpRight size={16}/></button></header>
  ...
  {account.loading?<p role="status">Ouverture de ton compte…</p>:account.user&&!profile?<div className="loyalty-empty"><p>Synchronisation de ton compte…</p><button onClick={account.refresh}>Réessayer</button><button onClick={logout}>Se déconnecter</button></div>:profile?<>
   ...
   <div className="account-dashboard"><div><BoutiqueCard profile={profile}/><p className="account-boutique-label">Ta carte de fidélité boutique · vêtements et accessoires</p><RewardStats profile={profile}/></div><article className="account-summary"><span className="loyalty-eyebrow">MEMBRE DU CERCLE</span><h2>@{profile.handle}</h2><div className="loyalty-numbers"><div><span>NIVEAU GLOBAL</span><strong>{economy?.global_level??'—'} <small>/ 150</small></strong></div><div><span>EXPÉRIENCE</span><strong>{economy?.xp??profile.xp} <small>XP</small></strong></div><div><span>COINS 3B</span><strong>{economy?.coins??0} <small>Coins</small></strong></div><div><span>FIDÉLITÉ</span><strong>{profile.points} <small>points</small></strong></div></div>{economy?.title&&<p className="account-progress-title"><strong>{economy.title}</strong>{Number(economy.prestige_level)>0?<> · Prestige {['','I','II','III'][Math.min(3,Number(economy.prestige_level))]}</>:null}</p>}{economy?.season&&<p className="account-progress-season">Saison active : <strong>{economy.season.label}</strong></p>}{economy?.prestige_eligible&&Number(economy?.prestige_level||0)<3&&<button className="loyalty-primary account-prestige-unlock" onClick={unlockPrestige} disabled={prestigeBusy}>{prestigeBusy?'Validation du Prestige…':'Débloquer Prestige '+['I','II','III'][Number(economy?.prestige_level||0)]}</button>}{economy?.next_level_xp!=null&&<p className="account-progress-next">Prochain niveau : {economy.next_level_xp} XP · Courbe {economy.xp_curve_version||'globale'}</p>}<p><ShieldCheck size={16}/> Compte synchronisé en ligne</p><p>{profile.country} · Depuis le {new Date(profile.created_at).toLocaleDateString('fr-FR')}</p><p className="account-id">N° membre : {profile.user_id.toUpperCase()}</p><button className="loyalty-primary" onClick={()=>goTo('loyalty')}>Mes cartes et avantages <ArrowUpRight size={16}/></button><button onClick={()=>goTo('games')}>Jouer et gagner de l’XP</button><button onClick={()=>goTo('passport')}>Voir mon passeport</button><button className="account-logout" onClick={logout}><LogOut size={16}/> Se déconnecter</button></article></div>

 ...
 if(view==='online'&&!onlineRoom)return <div className="dada3b-shell" role="dialog" aria-modal="true">
  ...
  <main className="dada3b-setup"><section className="dada3b-setup-card dada3b-online-setup">
   ...
   <><CountryPicker value={onlineCountry} onChange={setOnlineCountry}/>
    ...
    {onlineMode==='private'&&<label className="dada3b-field"><span>Nombre maximum</span><select value={maxPlayers} onChange={e=>setMaxPlayers(Number(e.target.value))}>{[2,3,4,5,6,7,8].map(v=><option key={v}>{v}</option>)}</select></label>}
    <button className="dada3b-primary" disabled={!account.user||onlineBusy} onClick={()=>onlineMode==='private'?onlineAction('create',{mode:'private',countryId:onlineCountry,maxPlayers,rules}):onlineMode==='join'?onlineAction('join',{code:roomCode,countryId:onlineCountry}):onlineMode==='spectate'?onlineAction('spectate',{code:roomCode}):onlineAction('queue',{mode:onlineMode,countryId:onlineCountry,rules})}>{onlineBusy?'Connexion…':onlineMode==='private'?'Créer le salon':onlineMode==='join'?'Rejoindre':onlineMode==='spectate'?'Observer':'Chercher un adversaire'}</button>
   </>}

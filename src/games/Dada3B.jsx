...
const TURN_PHASE_LABELS=Object.freeze({idle:'PRÊT',rolling:'LANCER',selectingPiece:'CHOIX',previewPiece:'APERÇU',movingPiece:'MOUVEMENT',resolvingCapture:'DUEL',cinematic:'CINÉMATIQUE',checkingWin:'NEXUS',extraTurn:'REJOUE',nextTurn:'TOUR SUIVANT'});
...
export default function Dada3B({saved,onClose,onCheckpoint}){
 ...
 const[turnPhase,setTurnPhase]=useState('idle'),[selectedPiece,setSelectedPiece]=useState(null);
 ...
 function beginLocal(){
  ...
  const next=createMatch(config,rules);recorded.current=false;setSelectedPiece(null);setLastSeats(config);setView('local');setTurnPhase('idle');adoptLocal(next);setTutorial(true);setTutorialStep(0);
 }
 function resumeLocal(){if(!restored||restored.status!=='playing')return;recorded.current=false;setSelectedPiece(null);setView('local');setRules(restored.rules);setTurnPhase(restored.pendingRoll!==null?'selectingPiece':'idle');adoptLocal(restored);setNotice('Partie restaurée depuis ta sauvegarde 3B.');}
 function replay(){if(!lastSeats)return;recorded.current=false;setSelectedPiece(null);setTurnPhase('idle');adoptLocal(createMatch(lastSeats,rules));}
 ...
 async function rollLocal(automated=false){
  ...
  setBusy(true);setSelectedPiece(null);setTurnPhase('rolling');setLegal([]);setNotice(countryFor(player.countryId).name+' lance le dé…');dadaTone('roll',sound);dadaHaptic('roll',haptic);
  ...
  const piece=automated?selectBotMove(rolled.match,finalRoll,rolled.match.turn,player.aiLevel||rules.aiLevel):null;
  if(piece!==null){await wait(180);await animateLocal(rolled.match,piece);return;}
  setLegal(moves);setSelectedPiece(null);setTurnPhase('selectingPiece');setBusy(false);setNotice('Choisis un guerrier, regarde sa position et sa destination, puis valide.');
 }
 function previewPiece(pieceIndex){if(busy||!currentLegal.includes(pieceIndex))return;setSelectedPiece(pieceIndex);if(!isOnline)setTurnPhase('previewPiece');setNotice('Guerrier sélectionné · vérifie sa position et sa destination puis valide.');}
 function cancelPreview(){setSelectedPiece(null);if(!onlineRoom)setTurnPhase('selectingPiece');setNotice('Choisis un autre guerrier.');}
 async function confirmPreview(){if(selectedPiece===null||busy||onlineBusy)return;const piece=selectedPiece;setSelectedPiece(null);if(onlineRoom){await onlineAction('move',{room:onlineRoom.id,revision:onlineRoom.revision,piece});return;}chooseLocal(piece);}
 function chooseLocal(pieceIndex){if(!match||busy||turnPhase==='resolvingCapture'||!match.pendingMoves?.includes(pieceIndex))return;setSelectedPiece(null);setTurnPhase('movingPiece');setBusy(true);animateLocal(match,pieceIndex);}
 ...
 const boardPieceAction=piece=>previewPiece(piece);
 ...
 const phase=selectedPiece!==null?'previewPiece':isOnline?(onlineBusy?'rolling':renderMatch.pendingRoll!==null?'selectingPiece':'idle'):turnPhase;
 ...
 return <div className="dada3b-shell dada3b-shell-v8" data-theme={renderMatch.rules?.boardTheme||'nexus'} role="dialog" aria-modal="true">
  ...
  <div className="dada3b-arena"><div className="dada3b-board-wrap" data-phase={phase}>{threeD&&!threeFailed?<Dada3DErrorBoundary fallback={fallbackBoard} onFail={()=>setThreeFailed(true)}><React.Suspense fallback={fallbackBoard}><Dada3BThree match={renderMatch} legal={currentLegal} selectedPiece={selectedPiece} motion={motion} blast={blast} onPiece={boardPieceAction} focusEvent={focus} loadout={cosmeticLoadout} cosmeticsByCountry={cosmeticsByCountry} onUnsupported={()=>setThreeFailed(true)}/></React.Suspense></Dada3DErrorBoundary>:fallbackBoard}<div className="dada3b-board-turn-chip" style={{'--country':turnCountry?.accent||'#6fe7f8'}}><span>TOUR</span><strong>{turnCountry?.flag} {turnCountry?.name}</strong><em data-own={ownsTurn}>{turnLabel}</em><i>{TURN_PHASE_LABELS[phase]||phase}</i></div><div className="dada3b-player-dice" data-phase={phase} style={{left:diceAnchor.left+'%',top:diceAnchor.top+'%','--country':turnCountry?.accent||'#6fe7f8'}}><span className="dada3b-player-dice-label">{turnCountry?.flag} {turnCountry?.code}</span><PremiumDice value={shownDice} rolling={(busy||onlineBusy)&&renderMatch.pendingRoll===null} skin={cosmeticLoadout?.dice_skin||'DADA_DICE_CORE'} country={turnCountry?.accent} pending={Boolean(renderMatch.pendingRoll)} adverse={Boolean(isOnline&&!selfTurn)} onClick={diceAction} disabled={diceDisabled}/></div>{(phase==='selectingPiece'||phase==='previewPiece')&&currentLegal.length>0&&<div className="dada3b-piece-picker" data-preview={selectedPiece!==null} style={{'--country':turnCountry?.accent||'#6fe7f8'}}><small>{selectedPiece===null?'1 · CHOISIS TON GUERRIER':'2 · VÉRIFIE PUIS VALIDE'}</small><div>{currentLegal.map(pieceIndex=>{const archetype=WARRIOR_ARCHETYPES[pieceIndex%WARRIOR_ARCHETYPES.length],selected=selectedPiece===pieceIndex;return <button key={pieceIndex} type="button" aria-pressed={selected} onClick={()=>previewPiece(pieceIndex)} disabled={busy||onlineBusy}><span>{WARRIOR_SIGILS[archetype]}</span><b>{WARRIOR_LABELS[archetype]}</b><em>{selected?'SÉLECTIONNÉ':'#'+(pieceIndex+1)}</em></button>;})}</div>{selectedPiece!==null&&<footer><button type="button" className="dada3b-preview-change" onClick={cancelPreview}>CHANGER</button><button type="button" className="dada3b-preview-confirm" onClick={confirmPreview}>VALIDER LE DÉPLACEMENT</button></footer>}</div>}</div>

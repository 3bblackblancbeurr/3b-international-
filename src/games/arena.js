...
export class Arena extends BaseGame {
  constructor(seed){super(seed);this.enemies=[];this.bullets=[];this.gems=[];this.powers={};this.level=1;this.xp=0;this.need=7;this.kills=0;this.guardians=0;this.spawnClock=0;this.shotClock=0;this.lightClock=0;this.dash=0;this.invulnerable=0;this.bossAt=40;this.bossIndex=0;this.title='Kaïs contre les Ombres';this.combo=0;this.comboTime=0;this.bestCombo=0;this.dashTime=0;this.direction={x:1,y:0};this.trail=[];}
  spawn(boss=false){const side=Math.floor(this.random()*4),i=this.bossIndex,type=Math.floor(this.random()*3),hp=boss?540+i*160:(type===2?60:27)+this.time*.16;const e={x:side<2?(side===0?10:W-10):this.random()*W,y:side>=2?(side===2?10:H-10):this.random()*H,hp,maxHp:hp,speed:boss?85+i*5:Math.min(type===1?175:145,(type===1?133:80)+this.random()*16+this.time*.035),boss,type,phase:this.random()*6,hit:0,shot:3+this.random()*2};if(distance(e,this.player)<140){e.x=W-e.x;e.y=H-e.y;}if(boss){e.country=COUNTRIES[i];e.index=i;this.bossIndex++;this.message='Gardien de '+e.country;this.effect(W/2,90,COLORS[i],this.message);}this.enemies.push(e);}
  ...
  action(){if(this.status!=='playing'||this.dash>0)return;this.dash=2.8;this.dashTime=.22;this.dashVector={...this.direction};this.invulnerable=.48;this.cue('dash');this.effect(this.player.x,this.player.y,'#85dcf4','Esquive');}
  choose(id){if(this.status!=='upgrade'||!this.choices.some(p=>p.id===id))return;this.powers[id]=(this.powers[id]||0)+1;if(id==='heart'){this.player.maxHp+=20;this.player.hp=Math.min(this.player.maxHp,this.player.hp+50);}this.status='playing';this.cue('upgrade');}
  update(dt,input={}){
    if(this.status!=='playing')return;this.tick(dt);const p=this.player;
    this.dash=Math.max(0,this.dash-dt);this.invulnerable=Math.max(0,this.invulnerable-dt);
    const length=Math.hypot(input.x||0,input.y||0);if(length)this.direction={x:input.x/length,y:input.y/length};
    this.dashTime=Math.max(0,this.dashTime-dt);this.trail=this.trail.filter(t=>(t.life-=dt)>0);
    if(this.dashTime>0){this.trail.push({x:p.x,y:p.y,life:.22});movePlayer(p,this.dashVector,dt,620);}else movePlayer(p,input,dt,205);
    this.comboTime=Math.max(0,this.comboTime-dt);if(!this.comboTime)this.combo=0;
    this.spawnClock-=dt;if(this.spawnClock<=0){const count=this.time>14?2:1;for(let n=0;n<count&&this.enemies.length<110;n++)this.spawn();this.spawnClock=Math.max(.48,.72-this.time/1600);}
    ...
    for(const e of this.enemies.filter(e=>e.hp<=0)){this.kills++;this.combo++;this.comboTime=3;this.bestCombo=Math.max(this.bestCombo,this.combo);this.score+=(e.boss?500:10)*Math.min(4,1+Math.floor(this.combo/10));this.cue(e.boss?'victory':'hit');this.gems.push({x:e.x,y:e.y,value:e.boss?12:1});this.effect(e.x,e.y,e.boss?COLORS[e.index]:'#759ba1');if(e.boss){this.guardians++;p.hp=Math.min(p.maxHp,p.hp+24);this.effect(e.x,e.y,'#efd076','Gardien libéré');}}
    ...
    for(const g of this.gems){const d=distance(g,p);if(d<105+(this.powers.magnet||0)*65){g.x+=(p.x-g.x)*dt*7;g.y+=(p.y-g.y)*dt*7;}if(d<23){this.xp+=g.value;g.collected=true;this.cue('collect');}}this.gems=this.gems.filter(g=>!g.collected).slice(-240);
  ...
  hud(){return [['Vie',`${Math.ceil(this.player.hp)} / ${this.player.maxHp}`],['Niveau',this.level],['Série',this.combo+' · ×'+Math.min(4,1+Math.floor(this.combo/10))],['Gardiens',`${this.guardians} / 8`],['Temps',`${Math.floor(this.time/60)}:${String(Math.floor(this.time%60)).padStart(2,'0')}`]];}
}

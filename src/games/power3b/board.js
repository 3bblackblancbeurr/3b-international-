...
for(let i=0;i<8;i++){
 ...
 list.push({id:'hq'+i,name:'QG '+NATIONS[i].name,type:'land',hq:i,x:h.x,y:h.y});
...
export function neighbors(id){
 if(id.startsWith('hq')){const i=+id.slice(2);return['front'+i,'front'+mod(i-1)];}
 if(id.startsWith('front')){const i=+id.slice(5);return['hq'+i,'hq'+mod(i+1),'front'+mod(i-1),'front'+mod(i+1),'sea'+i,'sea'+mod(i-1)];}
 if(id.startsWith('sea')){const i=+id.slice(3);return['sea'+mod(i-1),'sea'+mod(i+1),'front'+i,'front'+mod(i+1)];}
 return[];
}
export function pathDistance(from,to,domain,max=9){
 if(from===to)return 0;const q=[[from,0]],seen=new Set([from]);
 while(q.length){const[cur,d]=q.shift();if(d>=max)continue;for(const n of neighbors(cur)){if(seen.has(n))continue;const s=SECTOR_BY_ID.get(n);if(domain==='land'&&s.type!=='land')continue;if(domain==='sea'&&s.type!=='sea')continue;if(n===to)return d+1;seen.add(n);q.push([n,d+1]);}}
 return Infinity;
}

...
export default function Power3B({saved,onCheckpoint,onClose}){
 ...
 const[cinematic,setCinematic]=useState(null),[rules,setRules]=useState(false),[planSeconds,setPlanSeconds]=useState(180);
 ...
 const resolve=()=>{if(cinematic)return;const next=resolveTurn(game);setGame(next);setUnitIds([]);setMode('move');setSectorId('hq'+human);setPlanSeconds(180);checkpoint(next,next.winner!==null);
 ...
 useEffect(()=>{if(!started||game.winner!==null||cinematic||rules)return;if(planSeconds<=0){resolve();return;}const t=setTimeout(()=>setPlanSeconds(s=>Math.max(0,s-1)),1000);return()=>clearTimeout(t);},[started,game.round,game.winner,cinematic,rules,planSeconds]);

...
export default function Power3B({saved,onCheckpoint,onClose}){
 const canvas=useRef(null),scrollRef=useRef(null),pendingEvents=useRef([]),motionFrame=useRef(0);
 ...
 const[cinematic,setCinematic]=useState(null),[cineQueue,setCineQueue]=useState([]),[moveFrom,setMoveFrom]=useState(null),[rules,setRules]=useState(false),[planSeconds,setPlanSeconds]=useState(180);
 ...
 const resolve=()=>{if(cinematic||moveFrom)return;const previous=structuredClone(game),next=resolveTurn(game);setGame(next);setUnitIds([]);setMode('move');setSectorId('hq'+human);setPlanSeconds(180);checkpoint(next,next.winner!==null);
  pendingEvents.current=[...(next.lastResolution?.events||[])].filter(e=>['mega','battle','stalemate','flag','victory','penalty'].includes(e.type)||(e.type==='power'&&e.winner===human)).sort((a,b)=>(eventWeight[b.type]||0)-(eventWeight[a.type]||0)).slice(0,6);setMoveFrom(previous);
 };
 useEffect(()=>{if(!cinematic)return;const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;const ms=reduced?350:cinematic.type==='victory'?3200:['flag','mega'].includes(cinematic.type)?2400:1450;const t=setTimeout(()=>setCineQueue(q=>{const next=q[0]||null;setCinematic(next);return q.slice(1);}),ms);return()=>clearTimeout(t);},[cinematic]);
 useEffect(()=>{if(!started||game.winner!==null||cinematic||moveFrom||rules)return;if(planSeconds<=0){resolve();return;}const t=setTimeout(()=>setPlanSeconds(s=>Math.max(0,s-1)),1000);return()=>clearTimeout(t);},[started,game.round,game.winner,cinematic,moveFrom,rules,planSeconds]);
 useEffect(()=>{const c=canvas.current;if(!c)return;const ctx=c.getContext('2d');renderPowerBoard(ctx,game,{sectorId,unitIds,legalTargets:mode==='move'?legal:mode==='mega'?SECTORS.map(s=>s.id):[],previousState:moveFrom,progress:moveFrom?0:1});},[game,sectorId,unitIds,legal,mode,moveFrom]);
 useEffect(()=>{if(!moveFrom||!canvas.current)return;const ctx=canvas.current.getContext('2d'),reduced=matchMedia('(prefers-reduced-motion: reduce)').matches,start=performance.now(),duration=reduced?1:820;cancelAnimationFrame(motionFrame.current);const frame=now=>{const p=Math.min(1,(now-start)/duration);renderPowerBoard(ctx,game,{sectorId,unitIds,legalTargets:[],previousState:moveFrom,progress:p});if(p<1){motionFrame.current=requestAnimationFrame(frame);return;}setMoveFrom(null);const events=pendingEvents.current;pendingEvents.current=[];if(events.length){setCinematic(events[0]);setCineQueue(events.slice(1));}else setNotice('Manche résolue. Aucun conflit majeur.');};motionFrame.current=requestAnimationFrame(frame);return()=>cancelAnimationFrame(motionFrame.current);},[moveFrom,game]);
 useEffect(()=>{const key=e=>{if(e.key==='Escape')close();if((e.key==='Enter'||e.key===' ')&&started&&!cinematic&&!moveFrom){e.preventDefault();resolve();}};window.addEventListener('keydown',key);return()=>window.removeEventListener('keydown',key);});
 const canvasTap=e=>{if(cinematic||moveFrom||game.winner!==null)return;const r=e.currentTarget.getBoundingClientRect(),x=(e.clientX-r.left)/r.width*BOARD_W,y=(e.clientY-r.top)/r.height*BOARD_H;
 ...
 return <div className="power3b-shell" data-resolving={moveFrom?'true':'false'} role="dialog" aria-modal="true" aria-label="Power 3B">
  ...
  {started&&<><div className="power3b-status"><div><span>Nation</span><strong style={{color:NATIONS[human].primary}}>{NATIONS[human].name} · {NATIONS[human].value}</strong></div><div><span>Manche</span><strong>{game.round}</strong></div><div><span>Temps</span><strong>{String(Math.floor(planSeconds/60)).padStart(2,'0')}:{String(planSeconds%60).padStart(2,'0')}</strong></div><div><span>Power disponible</span><strong>{availablePower} / {me.power}</strong></div><div><span>Ordres</span><strong>{myOrders.length} / {MAX_ORDERS}</strong></div><div><span>QG capturés</span><strong>{me.flagsCaptured} / 7</strong></div></div>
   <div className="power3b-main">
    ...
    <aside className="power3b-command">
     ...
     <div className="power3b-units">{sectorUnits.length?sectorUnits.map(u=><button key={u.id} disabled={u.nation!==human||u.type==='flag'||game.orders.some(o=>o.unitIds?.includes(u.id))} className={unitIds.includes(u.id)?'selected':''} onClick={()=>toggleUnit(u.id)}><i style={{'--nation':NATIONS[u.nation].primary}}>{UNIT_TYPES[u.type].icon}</i><span><strong>{UNIT_TYPES[u.type].name}</strong><small>{NATIONS[u.nation].code} · P {UNIT_TYPES[u.type].strength} · M {UNIT_TYPES[u.type].move}</small></span></button>):<p>Aucune unité dans ce secteur.</p>}</div>
   ...
   <section className="power3b-orders"><div className="power3b-orders-head"><div><span>ORDRES PROGRAMMÉS</span><strong>{myOrders.length} / 5</strong></div><button className="power3b-resolve" disabled={cinematic||moveFrom||game.winner!==null} onClick={resolve}><Shield size={18}/> EXÉCUTER LA MANCHE <ChevronRight size={18}/></button></div>
  ...
  {rules&&<div className="power3b-modal"><div><button className="power3b-modal-close" onClick={()=>setRules(false)}><X/></button><span>RÈGLES POWER 3B</span><h2>Le système classique, étendu à huit nations.</h2><ol><li>Tu disposes de 3 minutes pour inscrire jusqu’à 5 ordres. Un déplacement concerne une seule pièce.</li><li>Trois petites pièces identiques dans un même secteur peuvent être échangées contre leur grande unité : Infanteries→Régiment, Tanks→Char lourd, Chasseurs→Bombardier, Destroyers→Croiseur.</li><li>Les huit commandements exécutent leurs ordres avant la résolution des combats.</li><li>Dans un secteur disputé, les valeurs de puissance s’additionnent. Le plus fort capture les pièces adverses ; en cas d’égalité au sommet, les pièces arrivées pendant la manche se replient.</li><li>Après les conflits, l’occupation de pays adverses rapporte jusqu’à 3 Power par manche. Les Power achètent les petites unités.</li><li>Le Méga-missile vaut 100 Power : son tir détruit toutes les forces du secteur visé et le missile est consommé.</li><li>Un drapeau ne peut être capturé qu’avec une Infanterie ou un Régiment présent au QG. Le dernier commandement actif gagne.</li></ol><div className="power3b-reference"><div><b>Infanterie</b><span>M2 · P2</span></div><div><b>Tank</b><span>M3 · P3</span></div><div><b>Chasseur</b><span>M5 · P5</span></div><div><b>Destroyer</b><span>M1 · P10</span></div><div><b>Régiment</b><span>M2 · P20</span></div><div><b>Char lourd</b><span>M3 · P30</span></div><div><b>Bombardier</b><span>M5 · P25</span></div><div><b>Croiseur</b><span>M1 · P50</span></div><div><b>Méga-missile</b><span>Portée libre · 100</span></div></div></div></div>}
  {cinematic&&<div className={'power3b-cinematic '+cinematic.type} onClick={()=>setCineQueue(q=>{const next=q[0]||null;setCinematic(next);return q.slice(1);})}><div className="power3b-cine-lines"/><div className="power3b-cine-core">{cinematic.type==='mega'&&<div className="power3b-shockwave"/>}<span>{cinematic.type==='victory'?'3B · VICTOIRE':'RÉSOLUTION · MANCHE '+(game.round-1)}</span><h2>{cinematic.title}</h2><p>{cinematic.detail}</p><small>Toucher pour continuer</small></div></div>}

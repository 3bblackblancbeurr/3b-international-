...
export function resolveTurn(source,options={}){
 ...
 applyNoCommandPenalty(state,state.humanNation,events);const orders=state.orders.filter(o=>o.nation===state.humanNation).slice(0,MAX_ORDERS);if(options.ai!==false)for(let n=0;n<8;n++)if(n!==state.humanNation&&state.nations[n].active)orders.push(...aiOrders(state,n));

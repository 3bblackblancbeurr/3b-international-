...
const makeSpoils=()=>({infantry:0,tank:0,fighter:0,destroyer:0,regiment:0,heavyTank:0,bomber:0,cruiser:0});
...
export function gameScore(state,nation=state.humanNation){
 ...
 const reserveForce=Object.entries(me.spoils||{}).reduce((sum,[type,count])=>sum+(UNIT_TYPES[type]?.strength||0)*count,0);return me.flagsCaptured*500+territory*25+(force+reserveForce)*10+me.power;
}
...
export function validatePowerSnapshot(value){
 ...
 if(Array.isArray(value.nations)&&value.nations.length===8)state.nations=state.nations.map((base,i)=>{
  const src=value.nations[i]||{},reserve={...base.reserve},spoils={...base.spoils};
  for(const key of Object.keys(reserve))reserve[key]=cap(Number.isFinite(src.reserve?.[key])?Math.trunc(src.reserve[key]):reserve[key],0,99);for(const key of Object.keys(spoils))spoils[key]=cap(Number.isFinite(src.spoils?.[key])?Math.trunc(src.spoils[key]):0,0,99);
  return{...base,active:src.active!==false,power:cap(Number.isFinite(src.power)?Math.trunc(src.power):0,0,9999),flagsCaptured:cap(Number.isFinite(src.flagsCaptured)?Math.trunc(src.flagsCaptured):0,0,7),reserve,spoils};
 });
 const seen=new Set();

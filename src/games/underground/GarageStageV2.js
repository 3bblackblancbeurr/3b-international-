...
export class GarageStageV2{
  constructor(canvas,{onError=()=>{},onInteract=()=>{}}={}){
    ...
    try{
      ...
      this.renderer.transmissionResolutionScale=coarse?.55:.75;
      ...
      this.change=()=>{this.dirty=true;this.canvas.dataset.cameraState='moving';this.request();};
      ...
      canvas.dataset.garageVersion='modern-showroom-v2';canvas.dataset.environment='pmrem-studio';canvas.dataset.cameraState='moving';
  ...
  resize(){
    ...
    this.camera.aspect=width/height;this.camera.updateProjectionMatrix();this.renderer.setSize(width,height,false);this.dirty=true;this.canvas.dataset.cameraState='moving';this.request();
  }
  ...
  setModel(model){
    ...
    this.model=model;model.position.y+=.001;this.scene.add(model);model.updateMatrixWorld(true);this.renderer.shadowMap.needsUpdate=true;this.dirty=true;this.canvas.dataset.cameraState='moving';this.request();
  }
  selectView(preset,{immediate=false}={}){
    ...
    this.controls.minDistance=closeup?.65:2.4;this.canvas.dataset.cameraState='moving';
    this.transition={position:new THREE.Vector3(...preset.camera),target:new THREE.Vector3(...preset.target),fov:preset.fov||38,started:performance.now()};
  ...
  zoom(factor){
    ...
    this.transition={position:this.controls.target.clone().add(offset),target:this.controls.target.clone(),fov:this.camera.fov,started:performance.now()};this.canvas.dataset.cameraState='moving';this.request();
  }
  setAutoRotate(enabled){this.controls.autoRotate=Boolean(enabled);this.canvas.dataset.cameraState='moving';this.dirty=true;this.request();}
  ...
  renderFrame(now){
    ...
    this.canvas.dataset.cameraState=moving||this.dirty?'moving':'idle';
    if(moving||this.dirty)this.request();else this.last=0;
  }
  dispose(){

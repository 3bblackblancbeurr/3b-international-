...
Le tuning mécanique couvre maintenant 18 familles : moteur, admission, ECU, carburant, échappement, turbo, intercooler, refroidissement, embrayage, transmission, différentiel, pneus, freins, suspension, aéro, allègement, nitro et électronique.
...
Le système de personnalisation est entièrement data-driven avant les futurs modèles 3D : plus de 45 slots et plus de 250 variantes dans 9 familles.
...
## Rendu 3D actuel
`ThreeRaceView.js` et `PremiumWorld.js` fournissent un banc de course Three.js/WebGL avec route humide, bâtiments, pluie, éclairage, ombres, ACES, caméra poursuite, Cercle Brisé, France Gold Master procédurale et proxies de voitures.

## Fichiers clés
- `data.js` : pays, progression, disciplines, événements, IA.
- `carModel.js` : physique, PI, 18 upgrades et tuning PRO.
...
- `Underground3B.jsx` : Nexus, territoires, garage, course et contrôles.
- `underground.test.js` + `underground-customization.test.js` : tests.

## Principe de production
Aucun modèle final de voiture ne doit être ajouté avant validation de la conduite, de la progression, du garage, de la personnalisation, des boss et du rythme visuel. Les futurs meshes 3D remplaceront les proxies sans modifier les systèmes de carrière et de tuning.

...
export class ThreeRaceView{
  ...
  buildWorld(){
    ...
    this.player=upgradeVehicleProxyV7(createModularVehicleProxy(this.vehicle));this.player.name='U3B_PlayerVehicle';attachVehicleCinematicFX(this.player,{accent:'#ff203c'});this.scene.add(this.player);
  ...
  dispose(){this.composer?.dispose?.();this.world?.surfaceV7?.dispose?.();this.environmentTarget?.dispose?.();this.pmrem?.dispose?.();this.scene.traverse(o=>{if(o.geometry)o.geometry.dispose?.();if(o.material){const ms=Array.isArray(o.material)?o.material:[o.material];ms.forEach(m=>m.dispose?.());}o.userData?.contactShadowTexture?.dispose?.();});this.renderer.dispose();}
}

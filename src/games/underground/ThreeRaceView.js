...
export class ThreeRaceView{
  ...
  applyFxPressure(level){
    this.fxPressure=level;if(this.bloom)this.bloom.enabled=level<2;
    const cinematic=this.world?.cinematic;if(cinematic?.stars)cinematic.stars.visible=level<2;if(cinematic?.shafts)cinematic.shafts.forEach(s=>s.visible=level<1);
    if(this.world?.speedAtmosphere?.lines)this.world.speedAtmosphere.lines.visible=level<2||this.frameEma<26;
    if(this.world?.key&&this.profile!=='ultra')this.world.key.castShadow=level<2;
  }
  adaptResolution(dt){
    const ms=dt*1000;this.frameEma=this.frameEma*.94+ms*.06;this.adaptTimer+=dt;if(this.adaptTimer<1.6)return;this.adaptTimer=0;let next=this.pixelRatio;
    const heavy=this.postFxEnabled?18.4:19.2,light=this.postFxEnabled?13.0:13.8;if(this.frameEma>heavy)next=Math.max(.66,next-.1);else if(this.frameEma<light)next=Math.min(this.maxPixelRatio,next+.08);
    const pressure=this.frameEma>27?2:this.frameEma>21.5?1:this.frameEma<17?0:this.fxPressure;if(pressure!==this.fxPressure)this.applyFxPressure(pressure);

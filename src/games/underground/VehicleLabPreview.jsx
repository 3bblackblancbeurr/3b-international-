...
export default function VehicleLabPreview({vehicle}){
  const canvas=useRef(null),sceneRef=useRef(null),vehicleRef=useRef(null),cameraRef=useRef(null),rafRef=useRef(0),drag=useRef({active:false,x:0,yaw:0}),[preset,setPreset]=useState('exterior');
  ...
  useEffect(()=>{
    ...
    let start=performance.now();const loop=now=>{const dt=Math.min(.05,(now-start)/1000);start=now;const proxy=vehicleRef.current;if(proxy){updateProxyRuntime(proxy,vehicle,{speedKph:0,time:now/1000});if(!drag.current.active)proxy.rotation.y+=dt*.12;}renderer.render(scene,camera);rafRef.current=requestAnimationFrame(loop);};rafRef.current=requestAnimationFrame(loop);
  ...
  useEffect(()=>{
    const camera=cameraRef.current,proxy=vehicleRef.current;if(!camera||!proxy)return;const state=inspectionState(vehicle,preset),p=state.preset;camera.position.set(...p.camera);camera.fov=p.fov;camera.updateProjectionMatrix();camera.lookAt(new THREE.Vector3(...p.target));proxy.userData.inspectionState=state;
  },[preset,vehicle]);

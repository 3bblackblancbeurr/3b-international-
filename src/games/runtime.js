...
export function createStepper(step = 1 / 60) {
  ...
  return {
    reset() { accumulated = 0; },
    get alpha() { return Math.max(0,Math.min(1,accumulated/step)); },
...
export function actionState(game, id) {
  ...
  if (id === 'maze') {
    ...
    return { label: nearby ? 'Ouvrir le passage' : 'Éclat de lumière', remaining: nearby ? 0 : game.flashCooldown, duration: game.pulseRecharge||9, unavailable: !nearby && game.lamp <= 12 };
  }

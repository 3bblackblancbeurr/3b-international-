  ...
  const projects = Array.isArray(input.projects)
    ? input.projects
      ...
      .map(project => ({
        ...
        scripts: Array.isArray(project.scripts) ? project.scripts.slice(0, 40).map((script, index) => ({
          id: String(script?.id || `script-${index + 1}`).slice(0, 80),
          name: String(script?.name || "Script").slice(0, 80),
          language: "javascript",

...
import {createHubRuntime,HUB_MISSIONS,resolveHubInteraction} from '../hub/runtime-three.js';
  ...
  if(state.zone==='sanctuary'&&hubRuntime){
    ...
    if(districtId&&districtId!==lastHubDistrictId){
      ...
      if(result.changed){
        ...
        if(result.completedMission){const reward=result.completedMission.importance==='major'?120:result.completedMission.importance==='normal'?75:45;state.xp+=reward;onMessage({text:'Mission terminée : « '+result.completedMission.title+' » · +'+reward+' XP Cité.'});}
        save();
      }
    }
  }

...
import {styleFor,tickWeaponState} from './weapon-styles.js';
import {createCinematicDirector,worldScene} from './cinematic-director.js';
import {simulationFrame} from './simulation-time.js';
...
import {createHubRuntime,HUB_MISSIONS,hubLivingState,resolveHubInteraction,settleHubSecrets} from '../hub/runtime-three.js';
import {activeHubMission,applyHubEvent} from '../hub/mission-runtime.js';
...
export function createOriginsScene(host,{initial,onSnapshot,onSave,onMessage,onLoad,onError}){
 ...
 const lightProfile=()=>renderQuality.profile()==='light';
 ...
 let regionalEncounter=false,lastHubDistrictId=null;
 const combatInput=createCombatInput(),director=createCinematicDirector();
 function film(kind,target){director.enqueue(worldScene(kind,state.zone,target||{x:position.x,y:ground(position,state.zone)+1.3,z:position.z}));cinematic=director.current;combatInput.clear();blur();path=[];autoTarget=null;snapshot();}
 const companionRuntime={readyAt:0},gamepadInput=createGamepadInput();
 let position={...state.position},wolf=createCompanion(position),combat=createCombat(),world,hubRuntime=null,time=0,lastSnapshot=0,lastSave=0,lastEvent=0,raf,dead=false,paused=false,loading=true,heading=Math.PI,yaw=0,pitch=.30,manual=0,moveYaw=null,path=[],routeSprintUntil=0,lastGroundTapAt=-Infinity,lastCameraTapAt=-Infinity,vision=0,visionCooldown=0,cinematic=null,autoTarget=null,oldEnemyState='patrol',stepTime=0,frameCount=0,frameTime=0,fps=0,zoneToken=0;
 ...
 const dangerCircle=new T.CircleGeometry(1,64),dangerCone=new T.CircleGeometry(1,48,Math.PI/2-Math.acos(.2),2*Math.acos(.2));const guardGeometry=new T.RingGeometry(1.25,1.4,40,1,-Math.PI/2-Math.acos(.35),2*Math.acos(.35)),guardMaterial=new T.MeshBasicMaterial({color:'#79dbed',transparent:true,opacity:.65,side:T.DoubleSide,depthWrite:false}),guardArc=new T.Mesh(guardGeometry,guardMaterial);guardArc.rotation.x=-Math.PI/2;guardArc.visible=false;scene.add(guardArc);const danger=new T.Mesh(dangerCircle,dangerMat);danger.rotation.x=-Math.PI/2;danger.visible=false;scene.add(danger);
 ...
 function save(final=false){if(document.hidden&&!final)return;state.position=safePosition(position,state.zone,state.flags);state.hp=combat.hp||100;return onSave(structuredClone(state));}
 ...
 function snapshot(){const target=objective(state),interactions=available(),nearest=interactions.filter(p=>distance(position,p)<(p.range||3.2)).sort((a,b)=>distance(position,a)-distance(position,b))[0];const hubDistrict=state.zone==='sanctuary'?hubRuntime?.district(position)||null:null;const living=state.zone==='sanctuary'?hubLivingState(state.hub,new Date(),hubDistrict?.id):null;const hub=state.zone==='sanctuary'?{district:hubDistrict,mission:activeHubMission(state.hub,HUB_MISSIONS),progress:structuredClone(state.hub),event:living?.event||null,events:living?.events||[],secrets:living?.secrets?.ids||[]}:null;onSnapshot({state:structuredClone(state),position:{...position},hub,wolf:{position:{...wolf.position},mode:wolf.mode,arrived:wolf.arrived,cooldown:Math.max(0,companionRuntime.readyAt-time),power:companionPower(state.avatar.companion).name},heading,yaw,vision:Math.max(0,vision),visionCooldown,objective:target,nearby:nearest?{id:nearest.id,name:nearest.name}:null,combat:{guarding:combat.guard||0,detached:combat.detached||0,recalling:combat.recalling||0,counter:combat.counterWindow||0,telegraph:combat.enemy.state==='windup'?Math.max(0,Math.min(1,1-combat.enemy.timer/(combat.enemy.windupDuration||combat.profile.windup))):0,areaAttack:!!(combat.enemy.pattern%2),lightCost:actionCost('light',state.avatar,state.xp,state.equipment),heavyCost:actionCost('heavy',state.avatar,state.xp,isCountry(state.zone)&&state.regions[state.zone].upgrade?'artisan':state.equipment),combo:combat.time-combat.comboAt<1.4?combat.combo:0,guard:combat.companionGuard||0,slow:combat.companionSlow||0,hp:combat.hp,stamina:combat.stamina,energy:combat.energy,enemy:combat.enemy.hp,enemyMax:combat.enemy.maxHp,enemyName:combat.enemy.name,state:combat.enemy.state,active:regionalEncounter||state.zone==='france'&&state.flags.echo&&state.flags.echo2&&!state.flags.defeated,dodge:combat.dodge,attack:combat.attack?.kind},fps,renderProfile:renderQuality.profile(),renderRatio:renderer.getPixelRatio(),drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,cameraDistance:camera.position.distanceTo(aim),selectedDistance:state.settings.zoom,loading,cinematic:cinematic?{type:cinematic.type,title:cinematic.title,line:cinematic.line,paused:cinematic.paused,reading:cinematic.reading}:null,route:!!path.length});}
 async function loadZone(arrival=false){
  const token=++zoneToken;impactLabels.dispose();combatInput.clear();regionalEncounter=false;if(isCountry(state.zone)){const hp=combat.hp;combat=createCombat(countryLayout(state.zone).encounter,state.zone);combat.hp=hp;}else if(combat.arena){const hp=combat.hp;combat=createCombat();combat.hp=hp;}loading=true;onLoad(true);path=[];autoTarget=null;lastHubDistrictId=null;director.clear();cinematic=null;vision=0;combat.attack=null;keys.clear();stick.x=stick.z=0;moveYaw=null;movementFrame.reset();motionSmoother.reset();world?.dispose();hubRuntime?.dispose();hubRuntime=null;world=createEnvironment(scene,state.zone,{onError,quality:lightMode?'light':'high',occlusion});if(state.zone==='sanctuary')hubRuntime=createHubRuntime(scene,{quality:state.settings.quality});audio.region(state.zone);const atmosphere=COUNTRIES[state.zone]||{sky:'#628fac',haze:'#c7d6d8'};sky.setRegion(atmosphere);scene.fog.color.set(atmosphere.haze);wolf=createCompanion(position);wolf.position=safePosition(wolf.position,state.zone,state.flags);smoothAim.set(position.x,ground(position,state.zone)+1.15,position.z);camera.position.set(position.x+Math.sin(yaw)*state.settings.zoom,3.4,position.z+Math.cos(yaw)*state.settings.zoom);camera.lookAt(smoothAim);
  try{await Promise.all([world.ready,actors.ready]);if(dead||token!==zoneToken)return;loading=false;onLoad(false);save();if(arrival&&!state.visited.includes(state.zone)){state.visited.push(state.zone);save();film('arrival');}snapshot();}catch(e){if(!dead&&token===zoneToken){loading=false;onLoad(false);onError('Impossible de terminer le chargement 3D. '+e.message);}}
 }
 function perform(id,extra={}){
  ...
  if(result.changed){Object.assign(state,result.save);if(state.zone!==before){position={...state.position};loadZone(true);}else{combat.hp=state.hp;save();}signal(result.cinematic||'quest');}
  if(result.encounter&&!regionalEncounter){const hp=combat.hp;combat=createCombat(countryLayout(state.zone).encounter,state.zone);combat.hp=hp;regionalEncounter=true;film('battle',{x:combat.enemy.x,y:1.8,z:combat.enemy.z});}if(id==='refuge'&&extra.topic==='rest'&&result.changed){combat.hp=100;combat.stamina=100;combat.energy=100;}
  ...
  if(result.cinematic&&result.changed)film(result.cinematic,result.cinematic==='returned'?{x:0,z:0,y:6}:{x:0,z:-54,y:2});
  if(result.changed&&extra.combatVictory)film('victory');
  if(result.changed&&extra.topic==='restore')film('garden');
  if(result.changed&&state.zone==='france'&&['echo','echo2'].includes(id)&&state.flags.echo&&state.flags.echo2&&!state.flags.defeated)film('battle',{x:combat.enemy.x,y:1.8,z:combat.enemy.z});
  snapshot();return result;
 }
 function available(){
  if(state.zone==='sanctuary')return [{id:'circle',...POINTS.circle},...WORLDS.map(w=>({...w,name:'Traverser · '+w.name})),...(hubRuntime?.interactions?.()||[])];
 ...
 function interact(id){if(paused||loading||director.current)return;const p=id?available().find(p=>p.id===id):available().filter(p=>distance(position,p)<(p.range||3.2)).sort((a,b)=>distance(position,a)-distance(position,b))[0];if(!p)return;if(state.zone==='sanctuary'&&String(p.type||'').startsWith('hub-')){const result=resolveHubInteraction(p,state.hub);if(result.changed){state.hub=result.progress;state.xp+=result.completedMission?(result.completedMission.importance==='major'?120:result.completedMission.importance==='normal'?75:45):0;}if(result.destination){position={...result.destination};state.position={...position};path=[];autoTarget=null;wolf=createCompanion(position);}if(result.changed||result.destination)save();if(result.message)onMessage({text:result.message});snapshot();return;}if(state.zone==='france'&&p.id.startsWith('walker')&&distance(position,p)<3.2){onMessage({speaker:'Habitant du quartier',text:state.flags.justice?'Les Archives ont retrouvé leurs voix. Je passe à l’atelier pour aider à réparer le quartier.':'La fontaine est notre point de rencontre. L’artisan, au sud-ouest, cherche quelqu’un pour son jardin.'});actors.talk(p.id);return;}if(isCountry(state.zone)){if(p.id==='country-encounter'&&regionalEncounter){onMessage({text:'L’épreuve est déjà en cours. Éloigne-toi vers le refuge pour te replier.'});return;}if(p.id==='country-landmark'){onMessage({text:'Tu es devant '+p.name+'. Une interprétation architecturale du monument, intégrée au Monde 3B.'});return;}const dialogue=countryDialogue(state.zone,p.id,'welcome',state);if(dialogue&&distance(position,p)<3.2){onMessage(dialogue);actors.talk(p.id);return;}}perform(p.id);}
 function action(kind){if(paused||loading||director.current)return;audio.start(state.settings);
 ...
 function navigate(id){const hubPoint=state.zone==='sanctuary'?hubRuntime?.interactions?.().find(p=>p.id===id):null;const p=hubPoint||(isCountry(state.zone)?countryLayout(state.zone).points.find(p=>p.id===id):POINTS[id])||WORLDS.find(w=>w.id===id);if(!p||paused||loading||director.current)return false;if(p.zone&&p.zone!==state.zone)return false;let destination=p;if(state.zone==='sanctuary'&&WORLDS.some(w=>w.id===id))destination={x:p.x*26/29,z:p.z*26/29};
  ...
  motionSmoother.reset();routeSprintUntil=0;path=route(position,destination,state.zone,state.flags,hubRuntime?.layout?.obstacles||[]);autoTarget=path.length?destination:null;return !!path.length;
 }
 const keyDown=e=>{if(paused||loading||director.current)return;if(['INPUT','TEXTAREA','SELECT','BUTTON'].includes(e.target.tagName))return;const k=e.key.toLowerCase();if([' ','arrowup','arrowdown','arrowleft','arrowright'].includes(k))e.preventDefault();if(e.repeat)return;keys.add(k);const commands={e:'interact',f:'wolf',v:'vision',j:'light',k:'heavy',r:'circle',g:'guard',' ':'dodge',c:'recenter'};if(commands[k])action(commands[k]);};
 ...
 const resize=()=>{const w=host.clientWidth,h=host.clientHeight;const next=lightProfile();if(next!==lightMode){lightMode=next;sun.shadow.mapSize.setScalar(lightMode?1024:2048);sun.shadow.map?.dispose();sun.shadow.map=null;}renderer.setPixelRatio(renderQuality.ratio(w,h,devicePixelRatio));renderer.setSize(w,h);camera.aspect=w/Math.max(1,h);camera.updateProjectionMatrix();};
 ...
 const visibility=()=>{if(document.hidden&&wasVisible)save(true);wasVisible=!document.hidden;gamepadInput.sample(navigator.getGamepads?.()||[],false);blur();audio.pause(document.hidden||paused);};document.addEventListener('visibilitychange',visibility);
  ...
  for(let simulationStep=0;simulationStep<timing.steps;simulationStep++){const dt=timing.step;director.tick(dt);cinematic=director.current;if(!cinematic)time+=dt;
   manual=Math.max(0,manual-dt);vision=Math.max(0,vision-dt);visionCooldown=Math.max(0,visionCooldown-dt);
   if(!cinematic){
    ...
    let speed=keys.has('shift')||pad.sprint||path.length&&routeSprintUntil>performance.now()?5.5:3.3;if(combat.guard>0)speed*=.4;if(combat.attack)speed*=combat.attack.kind==='light'?.82:.58;if(combat.dodge>0){speed=10;combat.dodgeDirection??=dodgeVector(x,z,heading);x=combat.dodgeDirection.x;z=combat.dodgeDirection.z;}
    const previous=position,next=advanceMotion({position,target:path[0]||null,route:path.slice(1)},{x,z},dt,speed,[],0,(p,dx,dz)=>move(p,dx,dz,state.zone,state.flags,.32,hubRuntime?.layout?.obstacles||[]));
    ...
    if(combat.event&&combat.event.id!==lastEvent){lastEvent=combat.event.id;const e=combat.event;signal(e.type,e.x,e.z);impactLabels.add(e,ground({x:e.x,z:e.z},state.zone));if(e.type==='impact')actors.enemy.action('Hit');if(['hurt','guardBreak'].includes(e.type))actors.hero.action('Hit');if(e.type==='defeat'){actors.enemy.action('Death');if(regionalEncounter){regionalEncounter=false;perform('country-encounter',{combatVictory:true});}else perform('defeated',{combatVictory:true});}}
    if(combat.hp<=0){combat=createCombat();combat.hp=100;regionalEncounter=false;position=isCountry(state.zone)?{...countryLayout(state.zone).points.find(p=>p.id==='refuge')}:{x:0,z:-48};state.position={...position};state.hp=100;wolf=createCompanion(position);path=[];save();onMessage({text:'Le loup t’a ramené à l’abri. Tes quêtes et tes souvenirs sont conservés.'});film('defeat');}
   }
   combat.loadout=state.avatar;combat.xp=state.xp;
   actors.tick(cinematic?0:dt,state,position,heading,motion,wolf,wMotion,combat,time,regionalEncounter);audio.tick(dt);
  }
  world?.update(state,vision>0,position,time);hubRuntime?.update(position,time,state.settings.quality);
  if(state.zone==='sanctuary'&&hubRuntime){
    const districtId=hubRuntime.district(position)?.id||null;
    if(districtId&&districtId!==lastHubDistrictId){
      lastHubDistrictId=districtId;
      const result=applyHubEvent(state.hub,{type:'arrive',id:districtId},HUB_MISSIONS);
      if(result.changed){
        state.hub=result.progress;
        if(result.completedMission){const reward=result.completedMission.importance==='major'?120:result.completedMission.importance==='normal'?75:45;state.xp+=reward;const secretState=settleHubSecrets(state.hub);state.hub=secretState.progress;onMessage({text:'Mission terminée : « '+result.completedMission.title+' » · +'+reward+' XP Cité.'+(secretState.unlocked.length?' Secret découvert : '+secretState.unlocked.map(s=>s.reward).join(' · ')+'.':'')});}
        save();
      }
    }
  }
  ...
  const filmStill=cinematic&&(cinematic.paused||cinematic.reading||matchMedia('(prefers-reduced-motion: reduce)').matches),filmYaw=cinematic?yaw+(filmStill?0:Math.sin(cinematic.elapsed*.3)*.12):yaw;const flat=Math.cos(pitch)*state.settings.zoom;desired.set(smoothAim.x+Math.sin(filmYaw)*flat,smoothAim.y+Math.sin(pitch)*state.settings.zoom,smoothAim.z+Math.cos(filmYaw)*flat);
  ...
  guardArc.visible=combat.guard>0&&!cinematic;guardArc.position.set(position.x,ground(position,state.zone)+.07,position.z);guardArc.rotation.z=heading;

...
export function createEnvironment(scene,zone,{onError=()=>{},quality='high',occlusion}={}){
 ...
 const workshopReady=new T.Group();root.add(workshopReady);workshopReady.name='Atelier restauré';workshopReady.visible=false;if(zone==='france'){box(wood,-19,1.05,34,1.7,.15,.65,0,workshopReady);for(const x of [-19.7,-18.3])box(bronze,x,.53,34,.1,1,.45,0,workshopReady);box(slate,-19,1.2,34,.5,.22,.35,0,workshopReady);plaque('ATELIER OUVERT',-19,1.8,34.4,1.8,workshopReady);}

...
export function createActors(scene,onError,playerAvatar){
 ...
 let hero=actor({avatar:playerAvatar||avatar,scale:1.02});const guardian=actor({avatar:{...avatar,style:'mystique',outer:'cape',fabricColor:'#d3c6a8',accentColor:'#ac9468'},scale:1.48}),enemy=actor({avatar:{...avatar,style:'sentinelle',outer:'cape',fabricColor:'#273742',accentColor:'#7ab4ce',skin:5},scale:1.48});
 const npcs=[
 ...
 let lastEnemy={x:0,z:-66},previousHP=130,deathTime=-100;
 const allReady=Promise.all([ready,...[0,1,2,3].map(i=>library.load('/world/living/traveller-'+i+'.glb'))]);
 return{get hero(){return hero;},setAvatar(next){return new Promise((resolve,reject)=>{const replacement=createLivingActor(library,{avatar:next,scale:1.02,onError:message=>{replacement.dispose();reject(Error(message));},onLoad:()=>{if(dead){replacement.dispose();reject(Error('Vue fermée'));return;}replacement.object.position.copy(hero.object.position);replacement.object.rotation.copy(hero.object.rotation);root.add(replacement.object);root.remove(hero.object);all.splice(all.indexOf(hero),1);hero.dispose();hero=replacement;all.push(hero);resolve();}});});},enemy,guardian,wolfRoot,npcs,ready:allReady,
  tick(dt,state,position,heading,motion,wolf,wMotion,combat,time,regionalEncounter=false){
   ...
   const france=state.zone==='france';guardian.object.visible=france;enemy.object.visible=regionalEncounter||isCountry(state.zone)&&combat.enemy.hp<=0&&time-deathTime<2.3||france&&state.flags.echo&&state.flags.echo2&&(!state.flags.defeated||time-deathTime<2.3);
   ...
   for(const n of npcs){n.a.object.visible=state.zone!=='sanctuary';const local=isCountry(state.zone)?countryLayout(state.zone).points.find(p=>p.id===n.id):null;let x=local?.x??n.x,z=local?.z??n.z,dx=0,dz=0;if(local&&n.id.startsWith('walker')){if(n.zone!==state.zone){n.zone=state.zone;n.walk={position:{x,z},target:null,route:[]};n.wait=1;n.visit=0;}n.wait=Math.max(0,(n.wait||0)-dt);if(!n.walk.target&&n.wait===0){const dest=countryNextStop(state.zone,n.visit++);const path=route(n.walk.position,{x:dest.x+1.2,z:dest.z+1.2},state.zone,{});n.walk.target=path[0];n.walk.route=path.slice(1);n.wait=3;}const old=n.walk.position;if(!n.pause)n.walk=advanceMotion(n.walk,{x:0,z:0},dt,1.1,[],0,(p,dx,dz)=>move(p,dx,dz,state.zone,{}));else n.pause=Math.max(0,n.pause-dt);x=n.walk.position.x;z=n.walk.position.z;dx=x-old.x;dz=z-old.z;}if(!local&&n.id.startsWith('walker')){const phase=time%(state.flags.justice?18:24),walking=phase<10;const offset=walking?Math.sin(phase/10*Math.PI)*3:0;x+=offset;dx=walking?Math.cos(phase/10*Math.PI)*.94*dt:0;}n.a.object.position.set(x,0,z);n.a.update(dt,dx,dz,Math.hypot(dx,dz));if(!dx)n.a.face(position.x-x,position.z-z,dt);}

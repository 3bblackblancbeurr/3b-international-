...
import {HUB_LAYOUT,HUB_MISSIONS} from '../hub/runtime-three.js';

import './origins.css';

const HUB_OBJECTIVE_TARGETS={
 first_steps:['hub-building:heritage_welcome','train:heritage_square','train:heritage_square'],
 first_echo:['hub-signal:archive_signal','hub-memory:archive_memory','hub-beacon:archive_beacon'],
...
function MapView({snapshot,large=false,onRoute}){
 ...
 return <svg className={'origins-map '+(large?'origins-map-large':'')} viewBox={s.zone==='sanctuary'?'-125 -125 250 250':(large||p.z< -78?'-115 -150 225 200':'-69 -84 138 130')} role="img" aria-label={'Plan du '+(s.zone==='sanctuary'?'Cité des Huit Héritages':(COUNTRIES[s.zone]?.district||'quartier France'))}>
  ...
  {s.zone==='sanctuary'?<><circle r="118" fill="#243a3e"/><polyline points={HUB_LAYOUT.trainStations.map(p=>p.x+','+p.z).join(' ')+' '+HUB_LAYOUT.trainStations[0].x+','+HUB_LAYOUT.trainStations[0].z} fill="none" stroke="#bfa873" strokeWidth="1.3" opacity=".75"/>{HUB_LAYOUT.ziplines.map(z=><line key={z.id} x1={z.from.x} y1={z.from.z} x2={z.to.x} y2={z.to.z} stroke="#54bedf" strokeWidth=".6" opacity=".65"/>)}{HUB_LAYOUT.buildings.map(b=><rect key={b.id} x={b.x-b.w/2} y={b.z-b.d/2} width={b.w} height={b.d} rx="1" fill={b.tier===0?'#9f8d68':'#53676d'} stroke="#c8b887" strokeWidth=".35"/>)}<circle r="39" fill="#476063" opacity=".72"/><circle r="22" fill="none" stroke="#998c6e" strokeWidth=".3"/><circle r="12" fill="none" stroke="#998c6e" strokeWidth=".3"/><path d="M -6 0 L 6 0" stroke="#e7d9b6" strokeWidth="2"/></>:<>
...
function Session({uid,goTo,onPrevious}){
 ...
 const s=snapshot?.state||initial,task=snapshot?.objective||objective(s),hubMission=snapshot?.hub?.mission,hubTarget=hubMission?HUB_OBJECTIVE_TARGETS[hubMission.id]?.[hubMission.step]:null,taskText=s.zone==='sanctuary'&&hubMission?(hubMission.title+' · '+(hubMission.currentObjective||'Mission terminée')):task.text,c=snapshot?.combat||{hp:100,energy:60,stamina:100},settings=s.settings;const weaponStyle=styleFor(s.avatar);const reinforced=isCountry(s.zone)?s.regions[s.zone].upgrade:s.equipment==='artisan';
 ...
 return <section className="origins" aria-label="3B ORIGINS — Le Cercle Brisé">
  ...
  {!panel&&<>
   ...
   <div className="origins-task"><span>{s.zone==='sanctuary'?(snapshot?.hub?.district?.name||'CITÉ DES HUIT HÉRITAGES'):(COUNTRIES[s.zone]?COUNTRIES[s.zone].name+' · '+COUNTRIES[s.zone].district:'FRANCE · JUSTICE')}</span><p>{taskText}</p>{s.zone==='sanctuary'&&snapshot?.hub?.event&&<small>ÉVÉNEMENT · {snapshot.hub.event.name} · {snapshot.hub.event.effect}</small>}{isCountry(s.zone)&&<small>{s.regions[s.zone].supplies} matériaux · Jardin {s.regions[s.zone].restored}/3 · {s.regions[s.zone].wins} victoires</small>}</div>
  ...
  {panel&&panel!=='start'&&<Panel title={panel==='awakening'?'L’Éveil de l’Héritage':panel==='character'?'Mon personnage':panel==='pause'?'L’aventure attend':panel==='settings'?'Paramètres':panel==='map'?'Les lieux retrouvés':panel==='inventory'?'Équipement et fragments':'Journal de '+s.avatar.name} close={panel==='character'?()=>setPanel('start'):close}>
   ...
   {panel==='journal'&&<><p className="origins-current">{taskText}</p><button onClick={()=>route(hubTarget||task.target)}>Rejoindre le prochain lieu</button>{s.zone==='sanctuary'&&<div className="origins-quests"><article><small>CITÉ VIVANTE · {s.hub.completed.length}/{HUB_MISSIONS.length} missions</small><h3>{hubMission?.title||'Cité des Huit Héritages'}</h3><p>{hubMission?.currentObjective||'Parle aux habitants, utilise les transports et découvre les quartiers pour démarrer une mission.'}</p><p>{s.hub.xp} XP Cité · {s.hub.coins} Coins · Train {s.hub.rides.train} · Bateau {s.hub.rides.boat} · Tyrolienne {s.hub.rides.zipline}</p><p>{s.hub.secrets.length} secret(s) découvert(s){snapshot?.hub?.event?' · Événement actif : '+snapshot.hub.event.name:''}.</p></article></div>}<div className="origins-quests">{isCountry(s.zone)?<article><small>VIE DU QUARTIER · {COUNTRIES[s.zone].name}</small><h3>Restaurer, se préparer et explorer</h3><ol><li>Récolter des matériaux au jardin.</li><li>Restaurer le jardin : {s.regions[s.zone].restored}/3.</li><li>Renforcer la tenue : {s.regions[s.zone].upgrade?'fait':'à faire'}.</li><li>Repousser l’Oubli pour ouvrir une nouvelle récolte : {s.regions[s.zone].wins} victoires.</li><li>Se reposer au refuge et découvrir le monument sur la carte.</li></ol><p>Tu peux poursuivre tes explorations et tes rencontres après la restauration.</p></article>:QUESTS.map(q=><article key={q.id}><small>{q.kind==='main'?'QUÊTE PRINCIPALE':'RENCONTRE'} · {s.rewards.includes(q.id)?'Accomplie':q.kind==='side'&&!(q.id==='garden'?s.flags.gardenAccepted:s.flags.memoryAccepted)?'À découvrir':'En cours'}</small><h3>{q.title}</h3><ol>{q.steps.map(text=><li key={text}>{text}</li>)}</ol></article>)}</div></>}

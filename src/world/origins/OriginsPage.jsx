...
function Session({uid,goTo,onPrevious}){
 ...
 useEffect(()=>{let live=true;try{scene.current=createOriginsScene(host.current,{initial,onSnapshot:v=>{if(live)setSnapshot(v);},onLoad:v=>{if(live)setLoading(v);},onError:e=>{if(live)setError(e);},onSave:s=>{const ok=persist(localStorage,uid,s);if(live)setSaveError(!ok);},onMessage:m=>{if(!live)return;setMessage(m);clearTimeout(messageTimer.current);if(!m.speaker&&!m.choices)messageTimer.current=setTimeout(()=>setMessage(null),6500);}});scene.current.pause(true);}catch(e){setError('Le rendu 3D ne démarre pas : '+e.message);setLoading(false);}return()=>{live=false;clearTimeout(messageTimer.current);scene.current?.destroy();scene.current=null;};},[uid,initial]);
 ...
 async function saveCharacter(avatar,looks){await scene.current.avatar(avatar,looks);close();scene.current?.startAudio();}
 ...
 return <section className="origins" aria-label="3B ORIGINS — Le Cercle Brisé">
  ...
  {!panel&&<>
   ...
   {snapshot?.cinematic&&<div className="origins-cinematic"><span>{snapshot.cinematic==='restored'?'LA JUSTICE REVIENT':snapshot.cinematic==='returned'?'UN FRAGMENT A RETROUVÉ SA PLACE':'LES DEUX PLATEAUX'}</span><button onClick={()=>scene.current?.skip()}>Passer</button></div>}
  ...
  {panel&&panel!=='start'&&<Panel title={panel==='character'?'Mon personnage':panel==='pause'?'L’aventure attend':panel==='settings'?'Paramètres':panel==='map'?'Les lieux retrouvés':panel==='inventory'?'Équipement et fragments':'Journal de '+s.avatar.name} close={panel==='character'?()=>setPanel('start'):close}>

   {panel==='character'&&<CharacterCreator avatar={s.avatar} looks={s.looks} onSave={saveCharacter} onCancel={()=>setPanel('start')}/>}
   {panel==='pause'&&<div className="origins-menu"><button className="origins-primary" onClick={close}><Play/>Reprendre</button><button onClick={()=>setPanel('character')}>Mon personnage</button><button onClick={()=>setPanel('journal')}><BookOpen/>Journal</button><button onClick={()=>setPanel('map')}><MapIcon/>Carte</button><button onClick={()=>setPanel('inventory')}><Diamond/>Équipement et fragments</button><button onClick={()=>setPanel('settings')}><Settings/>Paramètres</button><button onClick={()=>goTo('accueil')}><ArrowLeft/>Retour à l’application</button><details><summary>Version précédente</summary><p>L’ancienne aventure et ses sauvegardes restent accessibles séparément.</p><button onClick={onPrevious}>Ouvrir la version précédente</button></details><small>Sauvegarde sur cet appareil · personnage et progression.</small></div>}

   {panel==='settings'&&<div className="origins-settings"><label>Qualité graphique<select value={settings.quality} onChange={e=>change('quality',e.target.value)}><option value="auto">Automatique</option><option value="high">Élevée · ordinateur</option><option value="light">Allégée · mobile</option></select></label><label>Sensibilité caméra<input type="range" min=".25" max="2" step=".05" value={settings.sensitivity} onChange={e=>change('sensitivity',+e.target.value)}/></label><label>Distance caméra<input type="range" min="6" max="12" step=".5" value={settings.zoom} onChange={e=>change('zoom',+e.target.value)}/></label><label className="origins-check"><input type="checkbox" checked={settings.follow} onChange={e=>change('follow',e.target.checked)}/>Caméra derrière le sens de marche</label><label className="origins-check"><input type="checkbox" checked={settings.shake} onChange={e=>change('shake',e.target.checked)}/>Légères secousses d’impact</label><label>Ambiance musicale<input type="range" min="0" max="1" step=".05" value={settings.music} onChange={e=>change('music',+e.target.value)}/></label><label>Bruitages<input type="range" min="0" max="1" step=".05" value={settings.effects} onChange={e=>change('effects',+e.target.value)}/></label><p className="origins-muted">{snapshot?.fps||'—'} images/s observées · {snapshot?.drawCalls||0} appels de rendu. Cette mesure concerne ce navigateur.</p></div>}

...
function Session({uid,goTo,onPrevious}){
 ...
 return <section className="origins" aria-label="3B ORIGINS — Le Cercle Brisé">
  ...
  {panel&&panel!=='start'&&<Panel title={panel==='awakening'?'L’Éveil de l’Héritage':panel==='character'?'Mon personnage':panel==='pause'?'L’aventure attend':panel==='settings'?'Paramètres':panel==='map'?'Les lieux retrouvés':panel==='inventory'?'Équipement et fragments':'Journal de '+s.avatar.name} close={panel==='character'?()=>setPanel('start'):close}>
   ...
   {panel==='pause'&&<div className="origins-menu"><button className="origins-primary" onClick={close}><Play/>Reprendre</button><button onClick={()=>setPanel('character')}>Mon personnage</button><button onClick={()=>setPanel('awakening')}>Revoir mon éveil</button><button onClick={()=>setPanel('journal')}><BookOpen/>Journal</button><button onClick={()=>setPanel('map')}><MapIcon/>Carte</button><button onClick={()=>setPanel('inventory')}><Diamond/>Équipement et fragments</button><button onClick={()=>setPanel('settings')}><Settings/>Paramètres</button><button onClick={()=>goTo('accueil')}><ArrowLeft/>Retour à l’application</button><details><summary>Version précédente</summary><p>L’ancienne aventure et ses sauvegardes restent accessibles séparément.</p><button onClick={onPrevious}>Ouvrir la version précédente</button></details><small>Sauvegarde sur cet appareil · personnage et progression.</small></div>}

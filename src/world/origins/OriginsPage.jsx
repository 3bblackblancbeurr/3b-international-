...
function Session({uid,goTo,onPrevious}){
 ...
 return <section className="origins" aria-label="3B ORIGINS — Le Cercle Brisé">
  ...
  {!panel&&<>
   ...
   <div className="origins-task"><span>{s.zone==='sanctuary'?(snapshot?.hub?.district?.name||'CITÉ DES HUIT HÉRITAGES'):(COUNTRIES[s.zone]?COUNTRIES[s.zone].name+' · '+COUNTRIES[s.zone].district:'FRANCE · JUSTICE')}</span><p>{taskText}</p>{isCountry(s.zone)&&<small>{s.regions[s.zone].supplies} matériaux · Jardin {s.regions[s.zone].restored}/3 · {s.regions[s.zone].wins} victoires</small>}</div>

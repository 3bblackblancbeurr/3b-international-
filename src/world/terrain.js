...
export function landscapeItems(region,save){return [...worldItems(region,save),...serviceItems(region,save),...districtDestinations(region)].map(item=>{
 if(region==='hub'&&item.type==='portal'){const point=hubCountryGatePosition(hubPlan,item.id);return point?{...item,...point}:{...item,...toLandscape(region,item.x,item.z)};}
...
export function buildingSites(region,anchors=[]){
 if(region==='hub')return COUNTRIES.map((c,i)=>{const p=hubCountryGatePosition(hubPlan,c.id)||toLandscape(region,...c.portal);return{id:c.id,x:p.x+18,z:p.z-18,rotation:-.18+i*.2,variant:i,...buildingDimensions(c.id,i)};});

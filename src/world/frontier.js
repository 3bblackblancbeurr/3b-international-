...
export function frontierState(save,region=save.region){return save.adventure?.frontier?.[region]||{wood:0,stone:0,food:2,camp:0,forge:0,garden:0,expedition:0,harvest:[],jobs:[],activeJob:null};}
export function normalizeFrontier(input){const result={};for(const c of COUNTRIES){const s=input?.[c.id];if(!s)continue;result[c.id]={wood:int(s.wood,9999),stone:int(s.stone,9999),food:int(s.food,99),camp:int(s.camp,8),forge:int(s.forge,8),garden:int(s.garden,8),expedition:int(s.expedition),jobs:[...new Set(Array.isArray(s.jobs)?s.jobs:[])].filter(id=>['atelier','garden'].includes(id)),activeJob:['atelier','garden'].includes(s.activeJob)?s.activeJob:null,harvest:[...new Set(Array.isArray(s.harvest)?s.harvest:[])].filter(id=>RESOURCE_SITES.some(p=>p.id===id))};}return result;}

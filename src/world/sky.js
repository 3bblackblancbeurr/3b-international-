...
export function createWorldSky(renderer,onEnvironment){
 ...
 return{root,get environment(){return environment?.texture;},setRegion(biome){uniforms.photoReady.regional=!!biome.district;uniforms.photoReady.value=photograph&&!biome.district?1:0;uniforms.horizon.value.set(biome.haze).lerp(new THREE.Color('#b8cee0'),biome.district?.15:.78);uniforms.zenith.value.set(biome.sky).lerp(new THREE.Color('#245d98'),biome.district?.15:.85);},update(camera,time){camera.updateMatrixWorld();uniforms.inverseProjection.value.copy(camera.projectionMatrixInverse);uniforms.cameraWorld.value.copy(camera.matrixWorld);uniforms.time.value=time;},dispose(){stopped=true;photograph?.dispose();environment?.dispose();material.dispose();geometry.dispose();}};
}

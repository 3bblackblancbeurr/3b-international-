...
function WorldSession({uid,goTo}){
 ...
 function interact(item){
  ...
  if(item.type==='kais'){setPanel('kais');return;}
  if(item.type==='training'){setPanel('training');return;}
  if(item.type==='passport'){setPanel('passportHub');return;}
  if(item.type==='archives'){setPanel('archives');return;}
  if(item.type==='transit'){setPanel('transit');return;}
 ...
 const country=countryById[snapshot.region],stats=useMemo(()=>teamStats(save),[save]),regionItems=useMemo(()=>landscapeItems(snapshot.region,save),[snapshot.region,save]),outdoorEchoes=save.adventure.outdoorCredits,nextGate=COUNTRIES.find(c=>!save.seals.includes(c.id))||null;
 ...
 return <section ref={shell} className="world-shell" aria-label="Le Monde du 3B">
  ...
  {panel&&!fieldCombat&&<Modal kind={panel} title={({party:'Explorer ensemble',cafe:'Café des Liens',paris:'La vie du quartier',heritage:'Patrimoine et monde 3B',camp:'Mon refuge',collection:'Les compagnons du monde',sanctuary:'Un lieu pour ton groupe',team:'Ton équipe',atlas:'L’Atlas des huit portes',journal:'Journal d’exploration',gps:'Les échos du dehors',pause:'Une pause dans le voyage',encounterPause:'Rencontre suspendue',encounter:'Un écho te rencontre',final:'Le monde continue',story:'Un pays à reconstruire',wardrobe:'Ton style',avatar:'Ton personnage',arena:'L’Arène 3B',kais:'Kaïs · le Cercle Brisé',training:'Entraînement du Nexus',passportHub:'Maison du Passeport 3B',archives:'Archives du Cercle',transit:'Relais des huit Portes'})[panel]} onClose={closePanel} wide={['collection','atlas','journal','avatar','arena','archives','transit'].includes(panel)}>
   {panel==='kais'&&<div className="nexus-service-panel"><span className="world-kicker">GUIDE DU CERCLE</span><h3>{save.seals.length>=8?'Les huit fragments répondent.':'Le prochain passage t’attend.'}</h3><p>{save.seals.length>=8?'Tu as relié les huit royaumes. Le Nexus porte maintenant la mémoire de tout ce que tu as restauré.':'Kaïs suit l’état du Cercle Brisé. '+(nextGate?nextGate.name+' est le prochain royaume dont le sceau manque encore.':'Continue à explorer le Nexus.')}</p><div className="world-stat-row"><span>Royaumes visités <b>{save.visited.length}/8</b></span><span>Sceaux <b>{save.seals.length}/8</b></span><span>XP monde <b>{save.xp}</b></span></div><div className="world-actions">{nextGate&&<button className="world-primary" onClick={()=>navigateTo(nextGate.id)}>Me guider vers la Porte {nextGate.name}</button>}<button onClick={()=>setPanel('journal')}>Voir le journal</button></div></div>}
   {panel==='training'&&<div className="nexus-service-panel"><span className="world-kicker">ZONE SÛRE · AUCUN DÉGÂT</span><h3>Tester tes mouvements sans quitter le Hub.</h3><p>Les silhouettes du Nexus sont des simulations. Elles ne peuvent ni blesser ton personnage ni modifier ta progression.</p><div className="world-actions"><button className="world-primary" onClick={()=>scene.current?.trainingMove('strike')}>Tester une attaque</button><button onClick={()=>scene.current?.trainingMove('power')}>Tester un pouvoir</button><button onClick={()=>scene.current?.trainingMove('guard')}>Tester la garde</button></div><button onClick={()=>setPanel('team')}>Préparer mon équipe</button></div>}
   {panel==='passportHub'&&<div className="nexus-service-panel"><span className="world-kicker">IDENTITÉ · PROGRESSION · HÉRITAGE</span><h3>Ton Passeport existe dans le monde.</h3><p>Cette Maison relie ton voyage dans le Monde du 3B à ton identité 3B. Les royaumes visités et les sceaux restent visibles ici avant d’ouvrir le Passeport complet.</p><div className="world-stat-row"><span>Niveau <b>{levelFor(save.xp)}</b></span><span>Pays visités <b>{save.visited.length}/8</b></span><span>Sceaux <b>{save.seals.length}/8</b></span></div><div className="world-actions"><button className="world-primary" onClick={()=>goTo('passport')}>Ouvrir mon Passeport 3B</button><button onClick={()=>setPanel('atlas')}>Voir les huit Portes</button></div></div>}
   {panel==='archives'&&<div className="nexus-service-panel nexus-archives"><span className="world-kicker">NIVEAU INFÉRIEUR · MÉMOIRE DU NEXUS</span><h3>Les Archives gardent la trace des huit royaumes.</h3><p>La cour est réellement située sous le niveau principal du Hub. Les archives donnent une lecture rapide de ta progression sans transformer le Nexus en simple écran de menus.</p><div className="nexus-archive-grid">{COUNTRIES.map(c=><div key={c.id} className={save.seals.includes(c.id)?'is-restored':''}><span style={{color:c.color}}>{c.symbol}</span><strong>{c.name}</strong><small>{save.seals.includes(c.id)?'Sceau restauré':save.visited.includes(c.id)?'Monde découvert':'Porte à découvrir'}</small></div>)}</div><div className="world-actions"><button className="world-primary" onClick={()=>setPanel('journal')}>Lire le journal du Cercle</button><button onClick={()=>setPanel('atlas')}>Ouvrir l’Atlas</button></div></div>}
   {panel==='transit'&&<div className="nexus-service-panel"><span className="world-kicker">DÉPLACEMENT INTERNE DU HUB</span><h3>Choisir un secteur du Nexus.</h3><p>Le Relais ne contourne pas les Portes : il place un repère vers le secteur choisi. Les royaumes restent accessibles uniquement en traversant leur seuil.</p><div className="nexus-transit-grid">{COUNTRIES.map(c=><button key={c.id} style={{'--country-color':c.color}} onClick={()=>navigate(regionItems.find(i=>i.id===c.id))}><span>{c.symbol}</span><strong>{c.name}</strong></button>)}<button onClick={()=>navigate(regionItems.find(i=>i.id==='hub:atelier'))}><span>✦</span><strong>Atelier</strong></button><button onClick={()=>navigate(regionItems.find(i=>i.id==='hub:training'))}><span>◎</span><strong>Entraînement</strong></button><button onClick={()=>navigate(regionItems.find(i=>i.id==='hub:archives'))}><span>▣</span><strong>Archives</strong></button></div></div>}

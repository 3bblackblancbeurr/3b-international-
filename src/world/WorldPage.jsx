...
function WorldSession({uid,goTo}){
 ...
 const fieldCombat=save.region==='france'&&panel==='encounter'&&!!save.adventure.encounter&&!save.adventure.encounter.result&&!save.adventure.encounter.pact;
 ...
 return <section ref={shell} className="world-shell" aria-label="Le Monde du 3B">
  ...
  {!panel&&snapshot.region==='france'&&<>{!(snapshot.waypoint&&snapshot.remaining>7)&&<button className="paris-journal-link" onClick={()=>setPanel('paris')}>La vie du quartier</button>}{snapshot.interior&&<div className="paris-place">{snapshot.interior==='atelier'?'Atelier des Verrières':'Refuge des Liens'}</div>}</>}
  {panel&&!fieldCombat&&<Modal kind={panel} title={({cafe:'Café des Liens',paris:'La vie du quartier',heritage:'Patrimoine et monde 3B',camp:'Mon refuge',collection:'Les compagnons du monde',sanctuary:'Un lieu pour ton groupe',team:'Ton équipe',atlas:'L’Atlas des huit portes',journal:'Journal d’exploration',gps:'Les échos du dehors',pause:'Une pause dans le voyage',encounterPause:'Rencontre suspendue',encounter:'Un écho te rencontre',final:'Le monde continue',story:'Un pays à reconstruire',wardrobe:'Ton style',avatar:'Ton personnage',arena:'L’Arène 3B'})[panel]} onClose={closePanel} wide={['collection','atlas','journal','avatar','arena'].includes(panel)}>
   ...
   {panel==='paris'&&<ParisJournal save={save} onNavigate={navigateTo} onPanel={setPanel}/>}

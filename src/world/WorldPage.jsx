...
import {interactionDescription} from './interaction-actions.js';
...
function WorldSession({uid,goTo}){
 ...
 const [combatImpact,setCombatImpact]=useState(null),[npcDialogue,setNpcDialogue]=useState(null),[hubGuardianInfo,setHubGuardianInfo]=useState(null),[interactionMenu,setInteractionMenu]=useState(null),[storyCinematic,setStoryCinematic]=useState(null),[cinematicQueue,setCinematicQueue]=useState([]);
 ...
 async function syncCityProgress(){
  if(!uid||saveRef.current.region!=='hub')return;
  const before=saveRef.current.hub?.missions?.first_foundation?.completedObjectives||0;
  if(!act({type:'hubCitySync'}))return;
  const result=await sync(),after=result?.data?.hub?.missions?.first_foundation?.completedObjectives||before;
  if(after>before)announce('Ville 3B vérifiée par le serveur · Première Fondation '+after+'/3');
 }
 ...
 function closePanel(){setWorldRequested(true);const e=saveRef.current.adventure.encounter;if(e){if(['victory','recruited','missed','defeat'].includes(e.result)){finishEncounter();return;}setPanel(panel==='encounterPause'?'encounter':'encounterPause');return;}setNpcDialogue(null);setInteractionMenu(null);setPanel(null);}
 ...
 function runContextAction(item,entry){
  if(!item||!entry)return;
  if(entry.id==='primary'){setInteractionMenu(null);interact(item);return;}
  if(entry.id==='map'||entry.id==='transport-line'){setInteractionMenu(null);setPanel('atlas');return;}
  if(entry.id==='journal'){setInteractionMenu(null);setPanel('journal');return;}
  if(entry.id==='team'){setInteractionMenu(null);setPanel('team');return;}
  if(entry.id==='story'){setInteractionMenu(null);setPanel('story');return;}
  if(entry.id==='waypoint'){setInteractionMenu(null);navigate(item);announce(item.name+' · repère placé');return;}
  if(entry.id==='mission'){
   const ids=item.missionIds||[],targets=ids.map(id=>regionItems.find(candidate=>candidate.type==='hubMission'&&candidate.missionId===id)).filter(Boolean);
   const target=targets.find(candidate=>saveRef.current.hub?.missions?.[candidate.missionId]?.status==='active')||targets.find(candidate=>saveRef.current.hub?.missions?.[candidate.missionId]?.status==='available')||targets[0];
   setInteractionMenu(null);
   if(target){navigate(target);announce(target.name+' · repère de mission placé');}else announce(item.name+' n’a aucune mission disponible ici.');
   return;
  }
  if(entry.id==='guardian-topic'){
   const rule=GUARDIAN_VALUES[item.country];setInteractionMenu(null);
   if(rule){scene.current?.contextEffect?.('memory',item);announce(rule.name+' · '+rule.value+' · son histoire se révèle en avançant dans '+(countryById[item.country]?.name||item.country)+'.');}
   else announce('Ce personnage ne connaît pas encore de Gardien lié à son histoire.');
   return;
  }
  if(entry.id==='directions'){
   const target=regionItems.find(candidate=>candidate.type==='hubDistrict'&&candidate.district===item.district);setInteractionMenu(null);
   if(target){navigate(target);announce('Direction · '+target.name);}else announce('Aucun repère de quartier disponible.');
   return;
  }
  if(entry.id==='return-country'){setInteractionMenu(null);if(item.region)travel(item.region);return;}
  if(entry.id==='guardian-value'){
   setInteractionMenu(null);
   if(item.type==='hubGuardian'){setHubGuardianInfo(item);setPanel('guardianHub');}else setPanel('valueTrial');
   return;
  }
  if(entry.id==='memory-vision'){
   setInteractionMenu(null);scene.current?.contextEffect?.('memory',item);audio.current?.event('vision');announce(interactionDescription(item,entry.id,{save:saveRef.current}));return;
  }
  if(entry.id==='companion-scout'){
   setInteractionMenu(null);scene.current?.contextEffect?.('scout',item);scene.current?.waypoint?.(item,false);announce(interactionDescription(item,entry.id,{save:saveRef.current}));return;
  }
  if(entry.id==='prepare'){
   setInteractionMenu(null);const next=act({type:'prepare'});if(next)announce('Groupe préparé · le prochain affrontement profitera du quartier restauré.');return;
  }
  if(entry.id==='rest'){
   setInteractionMenu(null);
   if(item.type==='cafe'){setPanel('cafe');return;}
   if(item.type==='camp'){setPanel('camp');return;}
   announce('Tu prends un moment pour observer et reprendre ton souffle.');return;
  }
  if(entry.id==='avoid'){setInteractionMenu(null);announce(item.name+' · tu choisis de ne pas engager cette rencontre maintenant.');return;}
  if(entry.id==='final-recap'){setInteractionMenu(null);announce(interactionDescription(item,entry.id,{save:saveRef.current}));setPanel('journal');return;}
  if(['mission-details','building-functions','district-purpose','event-observe','inspect-resource','observe-enemy'].includes(entry.id)){
   setInteractionMenu(null);scene.current?.contextEffect?.('inspect',item);announce(interactionDescription(item,entry.id,{save:saveRef.current}));return;
  }
  setInteractionMenu(null);announce(interactionDescription(item,entry.id,{save:saveRef.current})||entry.label);
 }
 ...
 return <section ref={shell} className="world-shell" aria-label="Le Monde du 3B">
  ...
  <WorldHUD snapshot={snapshot} save={save} panel={panel} onPanel={setPanel} onInteract={entry=>runContextAction(snapshot.near,entry)} onMore={(item,actions)=>{setInteractionMenu({item,actions});setPanel('interaction');}} onGuide={()=>scene.current?.waypoint(snapshot.waypoint,true)} loaded={loaded&&!assetsLoading}/>
  ...
  {panel&&!fieldCombat&&<Modal kind={panel} title={({party:'Explorer ensemble',cafe:'Café des Liens',paris:'La vie du quartier',heritage:'Patrimoine et monde 3B',camp:'Mon refuge',collection:'Les compagnons du monde',sanctuary:'Un lieu pour ton groupe',team:'Ton équipe',atlas:'L’Atlas des huit portes',journal:'Journal d’exploration',gps:'Les échos du dehors',pause:'Une pause dans le voyage',encounterPause:'Rencontre suspendue',encounter:'Un écho te rencontre',final:'Le monde continue',story:'Un pays à reconstruire',wardrobe:'Ton style',avatar:'Ton personnage',arena:'L’Arène 3B',hubDialogue:npcDialogue?.item?.name||'Conversation',valueTrial:valueRule?'Épreuve · '+valueRule.value:'Épreuve du Gardien',guardianHub:hubGuardianInfo?.name||'Gardien',city3b:'Ville 3B',interaction:interactionMenu?.item?.name||'Actions contextuelles'})[panel]} onClose={closePanel} wide={['collection','atlas','journal','avatar','arena','city3b'].includes(panel)}>
   {panel==='city3b'&&<City3BPanel uid={uid} onLogin={()=>goTo('member')} onNotice={announce} onWorldCitySync={syncCityProgress}/>}
   {panel==='interaction'&&interactionMenu&&<div className="world-context-actions"><span className="world-kicker">ACTION · RÉACTION · CONSÉQUENCE</span><h3>{interactionMenu.item.name}</h3><p>Choisis ce que tu veux faire. L’action principale reste disponible directement avec E.</p><div className="world-actions">{interactionMenu.actions.map((entry,index)=><button key={entry.id} className={index===0?'world-primary':''} onClick={()=>runContextAction(interactionMenu.item,entry)}>{entry.label}<small>{entry.kind==='power'?'Pouvoir':entry.kind==='dialogue'?'Dialogue':entry.kind==='navigation'?'Repère / trajet':entry.kind==='story'?'Histoire':'Interaction'}</small></button>)}</div></div>}
   ...
   {panel==='cafe'&&<div className="paris-journal"><span className="world-kicker">LA TERRASSE DES VOYAGEURS</span><h3>Préparer la prochaine sortie</h3><p>Un panier du quartier contient trois provisions. Chaque expédition en consomme une.</p><p>Tu possèdes <strong>{save.shards} éclats</strong>.</p><button className="world-primary" disabled={save.shards<6} onClick={()=>{if(act({type:'provisions'}))announce('Panier préparé · +3 provisions · −6 éclats');}}>Acheter 3 provisions · 6 éclats</button><p>À court de provisions et d’éclats ? Le refuge conserve une solution de secours gratuite.</p><button onClick={()=>navigateTo('france:camp')}>Repérer le refuge</button></div>}
   ...
   {panel==='pause'&&<><div className="play-pause-heading"><small>LES HUIT PORTES</small><h3>Le voyage continue.</h3></div><div className="play-pause-nav"><button onClick={()=>setPanel('collection')}><Users/>Compagnons <small>{CHARACTERS.filter(p=>save.collection[p.id]).length} / {CHARACTERS.length}</small></button><button onClick={()=>setPanel('team')}><Users/>Équipe</button><button onClick={()=>setPanel('arena')}><Users/>Arène en ligne</button><button onClick={()=>setPanel('atlas')}><MapIcon/>Carte</button><button onClick={()=>setPanel('gps')}><Footprints/>Dehors</button></div><p className="world-save-status" role="status">{saveMessage}</p><p>{rewardMessage}</p>{!uid&&<p>Cette partie invitée reste sur cet appareil. La partie de ton compte possède sa propre progression synchronisée ; ta partie invitée reste disponible en te déconnectant.</p>}<p>Les XP monde renforcent ton aventure. Les XP et points de fidélité sont gagnés par le temps de jeu actif, avec les plafonds partagés de ton compte.</p><div className="world-options"><button onClick={()=>setPanel(null)}><Play size={19}/> Reprendre</button><button onClick={()=>{shell.current?.requestFullscreen?.().catch(()=>announce('Le plein écran n’est pas disponible dans ce navigateur.'));setPanel(null);}}><Maximize size={19}/> Plein écran</button><button onClick={()=>{scene.current?.toggleCamera();setPanel(null);}}><Compass size={19}/> Changer de vue</button><button onClick={()=>goTo('home')}><ArrowLeft size={19}/> Quitter le monde</button><button onClick={()=>goTo('world-origins')}>Retrouver ma partie Origins · sauvegarde séparée</button><button onClick={toggleSound}>{sound?<Volume2 size={19}/>:<VolumeX size={19}/>} Sons {sound?'activés':'désactivés'}</button><button onClick={sync}><RotateCcw size={19}/> Synchroniser</button><button onClick={exportSave}><Download size={19}/> Télécharger ma sauvegarde</button>{!uid&&<label className="world-import">Restaurer une copie invitée<input type="file" accept=".json,application/json" onChange={importSave}/></label>}<button onClick={()=>setPanel('avatar')}><Sparkles size={19}/> Personnage & armes</button><button onClick={()=>setPanel('journal')}><BookOpen size={19}/> Chapitres et Cité</button><button onClick={()=>goTo('member')}><Users size={19}/>{uid?'Mon compte 3B':'Jouer avec mon compte 3B'}</button></div><div className="world-quality"><label htmlFor="world-difficulty">Défis des rencontres</label><select id="world-difficulty" value={save.adventure.difficulty} onChange={e=>act({type:'difficulty',value:e.target.value})}><option value="adventure">Aventure · découvrir les huit pays</option><option value="expert">Expert · adversaires plus résistants et dangereux</option></select><p>La difficulté s’applique aux prochaines rencontres. Chaque gardien vaincu en Expert offre une récompense unique.</p></div><div className="world-quality"><label htmlFor="world-camera-follow">Caméra d’exploration</label><select id="world-camera-follow" value={snapshot.camera?.follow===false?'free':'follow'} onChange={e=>scene.current?.setCameraFollow(e.target.value==='follow')}><option value="follow">Suivre automatiquement mes déplacements</option><option value="free">Angle libre · orientation manuelle</option></select><p>Le regard manuel reste prioritaire. Le suivi reprend quand tu avances ; ton zoom reste identique en ville.</p></div><div className="world-quality"><label htmlFor="world-quality">Qualité graphique</label><select id="world-quality" value={quality} onChange={e=>{const mode=e.target.value;setQuality(mode);scene.current?.setQuality(mode);try{localStorage.setItem('3b-world-quality',mode);}catch{}}}><option value="fluid">LOW · performance mobile</option><option value="auto">MEDIUM · qualité adaptative</option><option value="detail">HIGH · détails & reflets</option></select><p>Le mode automatique ajuste la résolution pendant l’exploration. Les commandes et la vitesse restent identiques.</p></div><div className="world-quality world-audio-mixer"><label>Mixage audio 3B</label>{[['master','Général'],['music','Musique'],['ambience','Ambiance'],['sfx','Effets'],['voice','Voix']].map(([key,label])=><label key={key} className="world-audio-slider"><span>{label} <b>{Math.round(audioMix[key]*100)}%</b></span><input aria-label={'Volume '+label} type="range" min="0" max="1" step="0.01" value={audioMix[key]} onChange={e=>updateAudioMix(key,e.target.value)}/></label>)}<p>Les réglages sont conservés sur cet appareil. Les voix, la musique, l’ambiance et les effets restent séparés.</p></div><details><summary>Commandes et état du rendu</summary><p>Glisse à gauche pour avancer et à droite pour tourner la caméra. Un toucher court définit une destination. Clic droit pour la caméra, molette pour zoomer. Au clavier : flèches, ZQSD ou WASD ; E pour interagir ; Maj pour courir. Glisse plus loin pour courir au doigt. C change la vue.</p><p>{snapshot.fps||'—'} images/s · {snapshot.drawCalls||'—'} appels de rendu · résolution {snapshot.resolution||100} %. La performance dépend de ton appareil.</p></details></>}

...
function WorldSession({uid,goTo}){
 ...
 return <section ref={shell} className="world-shell" aria-label="Le Monde du 3B">
  ...
  {!panel&&snapshot.region==='france'&&<><button className="paris-journal-link" onClick={()=>setPanel('paris')}>La vie du quartier</button>{snapshot.interior&&<div className="paris-place">{snapshot.interior==='atelier'?'Atelier des Verrières':'Refuge des Liens'}</div>}</>}

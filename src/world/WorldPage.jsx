...
function WorldSession({uid,goTo}){
 ...
 async function refreshWorld(){const result=await loadWorld(uid);saveRef.current=result.data;setSave(result.data);scene.current?.setSave(result.data);return result;}
 ...
 return <section ref={shell} className="world-shell" aria-label="Le Monde du 3B">
  ...
  {fieldCombat&&<FieldEncounter save={save} act={act} onRetreat={finishEncounter} onPause={()=>setPanel('encounterPause')} snapshot={snapshot} onFieldAction={kind=>scene.current?.combatAction(kind)}/>}

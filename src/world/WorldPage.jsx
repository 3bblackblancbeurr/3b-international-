...
function WorldSession({uid,goTo}){
 ...
 const [combatImpact,setCombatImpact]=useState(null),[npcDialogue,setNpcDialogue]=useState(null),[hubGuardianInfo,setHubGuardianInfo]=useState(null),[cinematicQueue,setCinematicQueue]=useState([]);
 const canvas=useRef(null),shell=useRef(null),scene=useRef(null),saveRef=useRef(save),callbacks=useRef({}),ready=useRef(false),paused=useRef(false),activity=useRef(0),rewardEngine=useRef({status:'playing'}),watch=useRef(null),tracker=useRef(createWalkTracker()),walkRef=useRef(0),audio=useRef(null),dirty=useRef(false),saveTimer=useRef(null),noticeTimer=useRef(null),dialogueTurns=useRef(new Map()),cinematicSeen=useRef(new Set()),cinematicResumePanel=useRef(null);
 ...
 const finishCinematic=useCallback(()=>{if(activeCinematic)act({type:'cinematicSeen',key:activeCinematic.key});scene.current?.skipCinematic();setCinematicQueue(queue=>queue.slice(1));},[activeCinematic?.key,act]);
 ...
 useEffect(()=>{audio.current?.state(activeCinematic?.audioState||(fieldCombat?'combat':panel==='valueTrial'||panel==='guardianHub'?'guardian':panel==='journal'||panel==='story'?'mission':'exploration'));},[fieldCombat,panel,activeCinematic?.key]);
 ...
 useEffect(()=>{if(panel!=='encounter')setCombatImpact(null);else if(!activeCinematic&&saveRef.current.adventure.encounter&&!saveRef.current.adventure.encounter.result&&!saveRef.current.adventure.encounter.field)act({type:'fieldStart'});},[panel,loaded,activeCinematic?.key]);
 useEffect(()=>{const cinematicPaused=!!activeCinematic;paused.current=(!!panel&&!['encounter','gps'].includes(panel))||!!error||cinematicPaused;scene.current?.setPaused((!!panel&&!fieldCombat)||!!error||cinematicPaused);scene.current?.setPresentation(activeCinematic?'storyCinematic':panel);},[panel,error,loaded,fieldCombat,activeCinematic?.key]);
 useEffect(()=>{
  if(!activeCinematic||!loaded||assetsLoading)return;
  ...
  audio.current?.state(activeCinematic.audioState||'mission');
  audio.current?.speak(activeCinematic.title+'. '+activeCinematic.detail,{character:activeCinematic.voiceCharacter});
 },[activeCinematic?.key,loaded,assetsLoading]);
 ...
 return <section ref={shell} className="world-shell" aria-label="Le Monde du 3B">
  ...
  {panel&&!fieldCombat&&<Modal kind={panel} title={({party:'Explorer ensemble',cafe:'Café des Liens',paris:'La vie du quartier',heritage:'Patrimoine et monde 3B',camp:'Mon refuge',collection:'Les compagnons du monde',sanctuary:'Un lieu pour ton groupe',team:'Ton équipe',atlas:'L’Atlas des huit portes',journal:'Journal d’exploration',gps:'Les échos du dehors',pause:'Une pause dans le voyage',encounterPause:'Rencontre suspendue',encounter:'Un écho te rencontre',final:'Le monde continue',story:'Un pays à reconstruire',wardrobe:'Ton style',avatar:'Ton personnage',arena:'L’Arène 3B',hubDialogue:npcDialogue?.item?.name||'Conversation',valueTrial:valueRule?'Épreuve · '+valueRule.value:'Épreuve du Gardien',guardianHub:hubGuardianInfo?.name||'Gardien',city3b:'Ville 3B'})[panel]} onClose={closePanel} wide={['collection','atlas','journal','avatar','arena','city3b'].includes(panel)}>
   {panel==='city3b'&&<City3BPanel uid={uid} onLogin={()=>goTo('member')} onNotice={announce}/>}
   ...
   {panel==='avatar'&&<AvatarPanel uid={uid} save={save} act={act} onDone={finishAvatarReveal}/>}

...
function WorldSession({uid,goTo}){
 ...
 useEffect(()=>{let live=true;loadWorld(uid).then(result=>{if(!live)return;setSave(result.data);saveRef.current=result.data;dirty.current=!!result.needsSave;setSaveMessage(result.message);setLoaded(true);if(result.data.adventure.encounter)setPanel('encounter');});return()=>{live=false;};},[uid]);
 useEffect(()=>{
  if(!loaded)return;
 ...
 return <section ref={shell} className="world-shell" aria-label="Le Monde du 3B">
  ...
  {(!loaded||assetsLoading)&&!error&&<div className="world-loading" role="status">Préparation de Kaïs et des huit portes…</div>}

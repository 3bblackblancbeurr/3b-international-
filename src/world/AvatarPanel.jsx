...
export function AvatarPanel({save,act,onDone,uid=null}){
 ...
 return <div className="avatar-editor"><div className="avatar-preview"><ArenaStage avatar={draft}/><span>Glisse pour tourner</span></div><div className="avatar-workbench"><nav className="avatar-tabs" aria-label="Personnalisation du personnage"><button type="button" aria-pressed={section==='appearance'} onClick={()=>setSection('appearance')}>Apparence</button><button type="button" aria-pressed={section==='armory'} onClick={()=>setSection('armory')}>Armurerie</button></nav><form className="avatar-fields" onSubmit={e=>{e.preventDefault();const result=act({type:'avatar',avatar:draft});if(result){committed.current=true;clearAvatarDraft(uid);setMessage('Ton personnage est enregistré.');setReveal(normalizeAvatar(result.adventure?.avatar||{...draft,created:true}));}}}>
  ...
  <fieldset><legend>Corps avancé</legend>{BODY_CAPABILITIES.map(cap=><label className="avatar-range" key={cap.id}>{cap.label}{!cap.available&&<small> · asset 3D requis</small>}<input type="range" min="-1" max="1" step=".1" disabled={!cap.available} value={draft[cap.id]} onChange={e=>set(cap.id,Number(e.target.value))}/></label>)}</fieldset>
  <fieldset><legend>Visage avancé</legend>{FACE_CAPABILITIES.filter(cap=>['eyeSize','browHeight','mouthWidth','earSize'].includes(cap.id)).map(cap=><label className="avatar-range" key={cap.id}>{cap.label}{!cap.available&&<small> · asset 3D requis</small>}<input type="range" min="-1" max="1" step=".1" disabled={!cap.available} value={draft[cap.id]} onChange={e=>set(cap.id,Number(e.target.value))}/></label>)}<label className="avatar-range">Taches de rousseur <small>· texture 3D requise</small><input type="range" min="0" max="1" step=".1" disabled value={draft.freckles} onChange={e=>set('freckles',Number(e.target.value))}/></label><div className="avatar-pair"><label>Cicatrice<select disabled value={draft.scar} onChange={e=>set('scar',e.target.value)}><option value="none">Aucune · asset requis</option></select></label><label>Grain de beauté<select disabled value={draft.mole} onChange={e=>set('mole',e.target.value)}><option value="none">Aucun · asset requis</option></select></label></div><div className="avatar-pair"><label>Barbe<select disabled value={draft.beard} onChange={e=>set('beard',Number(e.target.value))}><option value="0">Aucune · asset séparé requis</option></select></label><label>Moustache<select disabled value={draft.mustache} onChange={e=>set('mustache',Number(e.target.value))}><option value="0">Aucune · asset séparé requis</option></select></label></div></fieldset>
  <fieldset><legend>Textile 3B · pipeline premium</legend><p>Ces slots sont sauvegardés et prêts à recevoir les vrais modèles 3D au fur et à mesure de leur production.</p>{Object.entries(AVATAR_ASSET_SLOTS).map(([slot,values])=><label key={slot}>{AVATAR_SLOT_LABELS[slot]}<select value={draft.assetSlots[slot]} onChange={e=>setDraft(d=>({...d,assetSlots:{...d.assetSlots,[slot]:e.target.value}}))}>{values.map(value=><option key={value} value={value}>{value}</option>)}</select></label>)}</fieldset>

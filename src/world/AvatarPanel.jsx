...
import {avatarStorageKeys,readAvatarPortrait,writeAvatarPortrait,AVATAR_PORTRAIT_EVENT} from './avatar-storage.js';
...
const mergePresetSources=(cloud,key)=>{const local=readPresets(key);return Array.from({length:3},(_,index)=>cloud?.[index]||local[index]||null);};
const readDraft=(base,key)=>{
 try{
...
export function AvatarPanel(props){return <AvatarPanelSession key={props.uid||'guest'} {...props}/>;}
function AvatarPanelSession({save,act,onDone,uid=null}){
 const keys=useMemo(()=>avatarStorageKeys(uid),[uid]);
 const {presets:PRESET_KEY,draft:DRAFT_KEY,portrait:PORTRAIT_KEY}=keys;
 const previewRoot=useRef(null),captureTimer=useRef(null),alive=useRef(true);
 const [initiallyCreated]=useState(()=>!!save.adventure.avatar.created);
 useEffect(()=>{alive.current=true;return()=>{alive.current=false;clearTimeout(captureTimer.current);};},[]);
 const initial=useMemo(()=>normalizeAvatar(save.adventure.avatar),[save.adventure.avatar]);
 const recovered=useMemo(()=>readDraft(initial,DRAFT_KEY),[initial,DRAFT_KEY]);
 ...
 const [revealed,setRevealed]=useState(false),[savedAvatar,setSavedAvatar]=useState(initial),[activeStep,setActiveStep]=useState('identity'),[presets,setPresets]=useState(()=>mergePresetSources(save.adventure.avatarPresets,PRESET_KEY));
 ...
 const [history,setHistory]=useState({past:[],future:[]}),[testing,setTesting]=useState(false),[testIndex,setTestIndex]=useState(0),[portrait,setPortrait]=useState(()=>readAvatarPortrait(uid)),[runtimeCapabilities,setRuntimeCapabilities]=useState({morphTargets:[],expressions:[]}),[expression,setExpression]=useState('neutral');
 ...
 const portraitChanged=()=>window.dispatchEvent(new CustomEvent(AVATAR_PORTRAIT_EVENT,{detail:{userId:uid}}));
 const capturePortrait=()=>{
  if(!uid){setMessage('Connecte ton compte pour créer un portrait Passeport personnel.');return;}
  clearTimeout(captureTimer.current);setPreviewFocus('face');setPreviewAngle(0);setPreviewPose('idle');setWeaponDrawn(false);setMessage('Préparation du portrait Passeport…');
  requestAnimationFrame(()=>{if(!alive.current)return;requestAnimationFrame(()=>{if(!alive.current)return;captureTimer.current=setTimeout(()=>{
   if(!alive.current)return;
   try{
    const canvas=previewRoot.current?.querySelector('canvas[data-character-preview="true"]');if(!canvas)throw Error('canvas');
    const data=canvas.toDataURL('image/jpeg',.82);if(!data||data.length<1200||!writeAvatarPortrait(uid,data))throw Error('capture');
    setPortrait(data);portraitChanged();setMessage('Portrait Passeport généré pour ton compte sur cet appareil.');
   }catch{setMessage('Le portrait n’a pas pu être généré sur cet appareil.');}
  },90);});});
 };
 const clearPortrait=()=>{try{localStorage.removeItem(PORTRAIT_KEY);}catch{}setPortrait('');portraitChanged();setMessage('Portrait local supprimé.');};
 useEffect(()=>{const timer=setTimeout(()=>{try{localStorage.setItem(DRAFT_KEY,JSON.stringify({version:3,updatedAt:Date.now(),avatar:draft}));}catch{}},320);return()=>clearTimeout(timer);},[draft,DRAFT_KEY]);
 useEffect(()=>{
  if(!testing)return;
 ...
 if(revealed)return <AvatarCinematic avatar={savedAvatar} onDone={()=>{setRevealed(false);onDone?.({firstCreation:!initiallyCreated,avatar:savedAvatar});}}/>;
 return <div className="avatar-editor-v2">
  <aside className="avatar-preview-v2">
   <div className="avatar-preview-stage" ref={previewRoot}><ArenaStage avatar={draft} focus={previewFocus} pose={previewPose} angle={previewAngle} weaponState={weaponDrawn?'preview':'world'} lighting={lighting} quality={previewQuality} expression={expression} onCapabilities={setRuntimeCapabilities} showAura={activeStep==='world'||activeStep==='finish'} showCompanion={activeStep==='world'||activeStep==='finish'}/></div>

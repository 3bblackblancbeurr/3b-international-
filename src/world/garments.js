...
export function fitGarments(model,recipe){const geometries=[],materials=[],cloth=[];
 const material=(color,metalness=0)=>{const m=new THREE.MeshStandardMaterial({color,roughness:metalness?.4:.92,metalness,side:THREE.DoubleSide});materials.push(m);return m;},fabric=material(recipe.cloth),accent=material(recipe.accentColor),leather=material(recipe.bootColor),gold=material('#c9ad75',.6);
 ...
 fabric.roughness=({cotton:.92,linen:1,satin:.38,leather:.55})[recipe.fabric]??.92;
 ...
 if(recipe.outer==='scarf'){const scarf=attach(new THREE.TorusGeometry(.13,.044,8,22),accent,at(chest,0,.075,.025),[1,1,1]);scarf.rotateX(Math.PI/2);box(accent,at(chest,.09,-.13,.18),[.085,.35,.032]);}
 if(recipe.outer==='cape'||recipe.outer==='apron'){const cape=recipe.outer==='cape',top=at(chest,0,cape?.035:-.02,cape?-.17:.25),length=cape?Math.max(.55,chest.y-waist.y+.3)*(recipe.capeLength||1):Math.max(.38,chest.y-waist.y+.12),geometry=new THREE.PlaneGeometry(cape?.5:.28,length,6,8);geometry.translate(0,-length/2,0);const positions=geometry.attributes.position;for(let i=0;i<positions.count;i++){const progress=-positions.getY(i)/length;positions.setX(i,positions.getX(i)*(1+progress*(cape?.35:.12)));positions.setZ(i,(cape?-1:1)*(.08*progress+.012*Math.cos(positions.getX(i)*35)));}geometry.computeVertexNormals();const piece=attach(geometry,cape?fabric:accent,top,[1,1,1]);cloth.push({mesh:piece,base:new Float32Array(positions.array),length});for(const s of [-1,1])box(gold,at(chest,s*.13,.04,.15),[.035,.035,.025]);if(!cape)box(fabric,at(chest,0,-.23,.303),[.13,.09,.025]);}
 return{update(time){for(const c of cloth){const p=c.mesh.geometry.attributes.position;for(let i=0;i<p.count;i++){const t=-c.base[i*3+1]/c.length;p.setZ(i,c.base[i*3+2]+Math.sin(time*3.2+t*4)*.027*t);}p.needsUpdate=true;}},dispose(){geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());}};
}

...
export function storyCinematicPresentation(event){
 ...
 const base={key:event.key,kind:event.kind,region,card:event.context?.card||guardian.rule?.card||null,audioState:'mission',voiceCharacter:'narrator',nextLabel:'Continuer'};
 switch(event.kind){
  ...
  case 'guardian-value-complete':return {...base,audioState:'guardian',voiceCharacter:guardian.rule?.card||'narrator',kicker:(guardian.rule?.value||'VALEUR').toUpperCase(),title:(guardian.rule?.value||'La valeur')+' est reconnue',detail:`${guardian.name} peut désormais se présenter. Tu n’affrontes pas seulement sa force : tu as compris la valeur qu’il protège.`};
  case 'guardian-intro':return {...base,audioState:'guardian',voiceCharacter:event.context?.card||guardian.rule?.card||'narrator',kicker:(guardian.rule?.value||'GARDIEN').toUpperCase()+' · GARDIEN',title:guardian.name+' se tient devant toi',detail:`${guardian.rule?.value?guardian.rule.value+' · ':''}${chapter?.guardian||'Observe son rythme, protège ton groupe et attends l’ouverture.'}`,nextLabel:'Commencer le combat'};
  case 'important-combat-result':{
   const victory=event.context?.result==='victory';
   return {...base,audioState:'guardian',voiceCharacter:event.context?.card||guardian.rule?.card||'narrator',kicker:victory?'LIBÉRATION':'REPLI',title:victory?`${guardian.name} est libéré`:'Le Gardien tient encore',detail:victory?`Le sceau de ${country?.name||region} répond. ${guardian.name} devient un allié permanent et la dernière restauration du pays peut commencer.`:'La défaite ne détruit pas ta progression. Prépare ton groupe, relis les indices et reviens.',nextLabel:victory?'Poursuivre la reconstruction':'Revenir dans le monde'};
  }
  case 'guardian-homecoming':return {...base,audioState:'guardian',voiceCharacter:guardian.rule?.card||'narrator',kicker:'RETOUR À LA CITÉ',title:guardian.name+' rejoint la Cité',detail:`${country?.name||'Le pays'} est reconstruite. ${guardian.name} est maintenant visible dans la Cité des Huit Héritages et reste lié à tes prochaines expéditions.`,nextLabel:'Entrer dans la Cité'};

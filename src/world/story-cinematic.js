...
export function storyCinematicPresentation(event){
 ...
 switch(event.kind){
  case 'world-opening':return {...base,countryName:'3B INTERNATIONAL',value:'Héritage',kicker:'SOUVENIR 00 · SOURCE INCONNUE',title:'LE MONDE DU 3B',detail:WORLD_NARRATIVE.player.hook+' Ta première mission n’est pas de sauver le monde : comprendre pourquoi ce souvenir t’a choisi.',nextLabel:'Prendre le contrôle'};
  ...
  case 'guardian-value-complete':return {...base,card:guardian.rule?.card||null,audioState:'guardian',voiceCharacter:guardian.rule?.card||'narrator',kicker:(guardian.rule?.value||'VALEUR').toUpperCase(),title:(guardian.rule?.value||'La valeur')+' est reconnue',detail:narrative?`${narrative.guardian.flaw} ${narrative.guardian.evolution}`:`${guardian.name} peut désormais se présenter. Tu as compris la valeur qu’il protège.`};

...
export function createLivingActor(library,{card,avatar,scale=1,onLoad,onError,weaponState='world'}={}){
 ...
 library.load(url).then(asset=>{
  ...
  if(!card){garments=fitGarments(model,recipe);if(avatar?.weapon)weaponModel=fitWeapon(model,avatar,{drawn:weaponState==='preview'});}mixer=new THREE.AnimationMixer(model);

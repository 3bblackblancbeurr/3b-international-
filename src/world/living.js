...
export function createLivingActor(library,{card,avatar,scale=1,onLoad,onError}={}){
 ...
 return {object,get ready(){return ready;},action(name){if(!ready)return;actionEnd=clock+(name==='Death'?2.5:name==='Hit'?.35:.75);transition(name,true);},face(dx,dz,dt){const target=Math.atan2(dx,dz);heading+=Math.atan2(Math.sin(target-heading),Math.cos(target-heading))*(1-Math.exp(-dt*16));object.rotation.y=heading;},
  update(dt,dx=0,dz=0,travelled=0){clock+=dt;garments?.update(clock);if(!mixer)return;const speed=dt>0?travelled/dt:0;if(clock>=actionEnd){const name=speed>.08?(speed/scale>4?'Run':'Walk'):'Idle';transition(name);if(speed>.08){this.face(dx,dz,dt);actions[name]?.setEffectiveTimeScale(Math.min(2.2,Math.max(.6,speed/scale/(name==='Run'?4.8:1.6))));}}mixer.update(Math.min(.1,dt));},
  reset(){heading=0;object.rotation.y=0;actionEnd=0;transition('Idle');},setColor(color){if(card)return;recipe.cloth=color||avatarRecipe(avatar).cloth;for(const m of personal)if(/ClothColor/.test(m.name))m.color.set(recipe.cloth);},dispose(){dead=true;garments?.dispose();pattern?.dispose();mixer?.stopAllAction();if(model){mixer?.uncacheRoot(model);model.traverse(o=>{if(o.isSkinnedMesh)o.skeleton.dispose();});}personal.forEach(m=>m.dispose());object.clear();}
 };
}

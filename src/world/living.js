...
export function createLivingActor(library,{card,avatar,scale=1,onLoad,onError}={}){
 ...
 const object=new THREE.Group(),personal=new Set();let model,mixer,garments,weaponModel,pattern,actions={},legActions={},legCurrent=null,current=null,dead=false,clock=0,actionEnd=0,heading=0,ready=false,combatPose={};
 ...
 library.load(url).then(asset=>{
  ...
  if(!card){garments=fitGarments(model,recipe);if(avatar?.weapon)weaponModel=fitWeapon(model,avatar);}mixer=new THREE.AnimationMixer(model);
 ...
 return {object,setCombat(value){combatPose=value||{};},get ready(){return ready;},action(name,duration){if(!ready)return;const timed=Number.isFinite(duration)&&duration>0;const length=timed?Math.max(.18,Math.min(3.5,duration)):(name==='Death'?2.5:name==='Hit'?.35:Math.min(3.5,Math.max(.5,actions[name]?.getClip().duration||.75)));actionEnd=clock+length;transition(name,true);if(timed&&actions[name])actions[name].setEffectiveTimeScale(actions[name].getClip().duration/length);},face(dx,dz,dt){const target=Math.atan2(dx,dz);heading+=Math.atan2(Math.sin(target-heading),Math.cos(target-heading))*(1-Math.exp(-dt*16));object.rotation.y=heading;},
  update(dt,dx=0,dz=0,travelled=0){if(dead)return;clock+=dt;garments?.update(clock);weaponModel?.update(clock,combatPose);if(!mixer)return;const speed=dt>0?travelled/dt:0;if(combatPose.guard>0&&actions.Guard){if(current!=='Guard')transition('Guard',true);actionEnd=clock+.1;}else if(current==='Guard'&&clock<actionEnd)actionEnd=clock;if(clock>=actionEnd){const name=speed>.08?(speed/scale>4?'Run':'Walk'):'Idle';transition(name);if(speed>.08){actions[name]?.setEffectiveTimeScale(Math.min(2.2,Math.max(.6,speed/scale/(name==='Run'?4.8:1.6))));}}if(speed>.08)this.face(dx,dz,dt);const gait=speed>.08?(speed/scale>4?'Run':speed/scale>2.4&&legActions.Jog?'Jog':'Walk'):'Idle';if(legActions[gait]){if(current==='Death'&&clock<actionEnd){for(const a of Object.values(legActions))a.stop();legCurrent=null;}else{if(legCurrent!==gait){const next=legActions[gait];next.reset().play();if(legActions[legCurrent])legActions[legCurrent].crossFadeTo(next,.18,false);legCurrent=gait;}if(speed>.08)legActions[gait].setEffectiveTimeScale(Math.min(2,Math.max(.65,speed/scale/(gait==='Run'?4.8:gait==='Jog'?3.2:1.6))));}}mixer.update(Math.min(.1,dt));},

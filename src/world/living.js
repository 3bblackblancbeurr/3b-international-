...
import {FACE_CAPABILITIES} from './avatar-capabilities.js';
...
function applyFaceTargets(mesh,recipe,morphTargets){
 ...
 for(const cap of FACE_CAPABILITIES){
...
export function createLivingActor(library,{card,avatar,scale=1,onLoad,onError,weaponState='world'}={}){
 ...
 function applyAvatar(nextAvatar,{forceGarments=false}={}){
  ...
  model.traverse(o=>{
   ...
   if(o.morphTargetDictionary){applyFaceTargets(o,recipe,morphTargets);if(['Blink_L','Blink_R','EyeBlink_L','EyeBlink_R'].some(key=>o.morphTargetDictionary[key]!==undefined)&&!blinkMeshes.includes(o))blinkMeshes.push(o);}
  });

...
const MapGround=memo(function MapGround({field,hub=false}){
 const ground=hub?'#07131a':'#253f37',groundStroke=hub?'#d6b46a38':'#b4c5a330',water=hub?'#0b4f70':'#407c89',waterStroke=hub?'#63dfff88':'#769d9a',road=hub?'#5a6470':'#768567',street=hub?'#d6b46a':'#ab9e82',building=hub?'#18262f':'#ddd0ad',buildingStroke=hub?'#d6b46a88':'#554d3f';
 return <><circle r={field.radius} fill={ground} stroke={groundStroke}/>
  {field.fields.map((f,i)=><rect key={'f'+i} x={f.x-f.w/2} y={f.z-f.h/2} width={f.w} height={f.h} fill={hub?'#10231f':'#728056'} stroke={hub?'#62c99b22':'#a9ab711f'} transform={`rotate(${-f.rotation*180/Math.PI} ${f.x} ${f.z})`}/>)}
  <circle cx={field.lake.x} cy={field.lake.z} r={field.lake.r} fill={water} stroke={waterStroke} strokeWidth="1.5"/>
  {field.roads.map((r,i)=><polyline key={i} points={r.points.map(p=>p.x+','+p.z).join(' ')} fill="none" stroke={r.kind==='street'?street:road} strokeWidth={r.width} strokeLinejoin="round" strokeLinecap="round"/>)}
  {field.squares.map((p,i)=><circle key={'s'+i} cx={p.x} cy={p.z} r={p.r} fill={hub?'#17242b':'#ab9e82'} stroke={hub?'#d6b46a55':'none'}/>)}
  {[...field.buildings,...field.civic,...field.paris].map((b,i)=><rect key={'b'+i} x={b.x-b.width/2} y={b.z-b.depth/2} width={b.width} height={b.depth} rx=".4" transform={`rotate(${-b.rotation*180/Math.PI} ${b.x} ${b.z})`} fill={building} stroke={buildingStroke} strokeWidth=".5"/>)}
 </>;
});

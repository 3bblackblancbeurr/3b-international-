...
const CinematicGradeShader={
 ...
 fragmentShader:`
  ...
  void main(){
   ...
   color.r=mix(color.r,right.r,.008*intensity);
   color.b=mix(color.b,left.b,.008*intensity);
   float streak=(max(0.,luma(left)-.74)+max(0.,luma(right)-.74))*.5;
   color+=mix(accent,secondary,.58)*streak*.045*flare*intensity;
   vec2 centered=uv-.5;
   float vig=(1.-smoothstep(.28,.86,length(centered*vec2(1.08,.88))));
   color*=mix(.87+vig*.13,1.,1.-intensity*.42);
   float g=(hash(floor(uv*resolution)+floor(time*29.))-0.5)*.0075*grain*intensity;
   color+=g;

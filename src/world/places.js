...
export function addLivingPlaces(models,region,root,height){
 const groups=[],materials=[],living=[];
 ...
 return{groups,update(save){const active=region==='hub'||chapterState(save,region).restored>=2;for(const entry of living){entry.material.color.copy(entry.color);if(!active)entry.material.color.lerp(new THREE.Color('#b19c74'),.85);}},dispose(){materials.forEach(m=>m.dispose());}};
}

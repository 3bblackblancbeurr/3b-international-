...
export const WorldHUD=memo(function WorldHUD({snapshot,save,panel,onPanel,onInteract,onGuide,loaded}){
 ...
 const name=near?.id==='nexus'?'Entrer dans le Nexus':near?.type==='story'?near.name.split(' · ')[0]:near?.id==='hub'?'Retour à Cité Origine':near?.type==='beacon'?(near.done?'Souvenir retrouvé':'Recueillir'):near?.type==='guardian'?'Défier le gardien':near?.name;
 ...
 return <div className={'play-hud'+(snapshot.waypoint&&snapshot.remaining>7?' has-waypoint':'')+(panel?' is-hidden':'')} aria-hidden={panel?true:undefined}>
  ...
  <div className="world-district">{snapshot.district||country?.name||'Cité Origine'}</div>
  ...
  {arrival&&loaded&&<div className="play-arrival" key={snapshot.region}><span>{country?'MONDE-PAYS':'MONDE DU 3B · ULTRA MAP'}</span><h1>{country?.title||'Cité Origine'}</h1><i/>{country&&<p className="arrival-landmark">{HERITAGE[country.id]?.name}</p>}</div>}
  ...
  {near&&<button key={near.id} className="play-interaction" onClick={near.id==='nexus'?enterNexus:onInteract}><Icon size={20}/><span>{name}</span><kbd>E</kbd></button>}

...
import {contextualActions} from './interaction-actions.js';

export const WorldHUD=memo(function WorldHUD({snapshot,save,panel,onPanel,onInteract,onMore,onGuide,loaded}){
 const country=countryById[snapshot.region],near=snapshot.near,home=frontierState(save,snapshot.region),actions=contextualActions(near,{save,snapshot}),primary=actions[0];
 ...
 const Icon=near?.type==='portal'?DoorOpen:near?.type==='story'||near?.type==='echo'||near?.type==='hubNpc'?MessageCircle:near?.type==='guardian'||near?.type==='patrol'?Swords:Sparkles;
 const name=primary?.label||(near?.type==='story'?near.name.split(' · ')[0]:near?.id==='hub'?'Nexus':near?.name);
 ...
 return <div className={'play-hud'+(snapshot.waypoint&&snapshot.remaining>7?' has-waypoint':'')+(panel?' is-hidden':'')} aria-hidden={panel?true:undefined}>
  ...
  {near&&<div className="play-interaction-cluster" key={near.id}><button className="play-interaction" onClick={()=>onInteract(primary)}><Icon size={20}/><span>{name}</span><kbd>E</kbd></button>{actions.length>1&&<button className="play-interaction-more" aria-label={'Autres actions · '+near.name} title="Autres actions" onClick={()=>onMore?.(near,actions)}>•••</button>}</div>}

...
export function createLandscape(models,region,save,onError=console.error){
 ...
 if(hub){
  ...
  const core=toLandscape(region,0,-3),premiumPlatform=addHeritagePlatform({root,shape,geo,mat,height,owned,center:core});decorations.push({premiumPlatform});
 ...
 const dynamic=[genericBuildings,regional.group,sharedCamp,paris.group,...(heritage?[heritage.root]:[]),...frontierGroups.map(p=>p.group),...resourceGroups.map(p=>p.group),...authored.groups,...stages.map(s=>s.group),...decorations.flatMap(d=>[d.landmark,d.ruin,d.garden,d.workshop,d.grove,d.premiumPlatform?.dynamicRoot].filter(Boolean)),...residents.filter(r=>r.parent===root).map(r=>r.hero.object)];

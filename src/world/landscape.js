...
export function createLandscape(models,region,save,onError=console.error){
 const root=new THREE.Group(),owned=[],materials=new Map(),collisions=[],residents=[],stages=[],decorations=[],frontierGroups=[],resourceGroups=[],surfaceWetness={value:.06},surfaceDaylight={value:1};
 ...
 return{root,setParty,get architectureDiagnostics(){return regional.diagnostics;},ready:Promise.all([paris.ready,regional.ready]),get interior(){return paris.interior;},updateDistrict(camera,position){paris.tick(camera,position);regional.tick(camera,position);},renderWaterReflection(renderer,scene,camera,time){return premiumWater.renderReflection(renderer,scene,camera,time);},cinematicFocus(x,z,action='Talk',time=0){let best=null,dist=Infinity;for(const r of residents){const d=Math.hypot(r.hero.object.position.x-x,r.hero.object.position.z-z);if(d<dist){dist=d;best=r;}}if(best&&dist<18){best.hero.object.visible=true;best.hero.action?.(action);best.nextGesture=time+4;return true;}return false;},ground:terrain,collisions,height,field,update,setQuality(mode){meadow.setQuality(mode);premiumWater.setQuality(mode);soil.setQuality?.(mode);microDetails.setQuality?.(mode);premiumLighting.setQuality?.(mode);buildingContact.setQuality?.(mode);},setWeather(value){surfaceWetness.value=value==='storm'?1:value==='heavy_rain'?.82:value==='rain'?.55:value==='fog'?.22:.06;premiumWater.setWeather(value);soil.setWeather?.(value);microDetails.setWeather?.(value);architecture.setWeather?.(value);buildingContact.setWeather?.(value);},setDaylight(value){surfaceDaylight.value=Math.max(0,Math.min(1,Number(value)||0));premiumWater.setDaylight(value);soil.setDaylight?.(value);microDetails.setDaylight?.(value);premiumLighting.setDaylight?.(value);},updateCamera:occlusion.update,tick(time,dt,position){meadow.tick(reduceWind?0:time,position);flora.tick(reduceWind?0:time);premiumWater.update(time);for(const r of residents){r.hero.object.visible=r.parent.visible&&(r.route?Math.min(...r.route.points.map(p=>Math.hypot(position.x-p.x,position.z-p.z)))<38:Math.hypot(position.x-r.x,position.z-r.z)<46);if(r.hero.object.visible){if(r.route){const activity=residentSchedule(r.route,time,r.kind),x=activity.position.x,z=activity.position.z,dx=x-r.hero.object.position.x,dz=z-r.hero.object.position.z;r.hero.object.position.set(x,height(x,z),z);r.hero.update(dt,dx,dz,Math.hypot(dx,dz));if(activity.activity&&time>r.nextGesture){r.hero.action?.(activity.activity);r.nextGesture=time+3.5;}}else{r.hero.update(dt);if(time>r.nextGesture){r.hero.action?.(r.kind==='artisan'?'Work':'Talk');r.nextGesture=time+3+(r.phase%4);}if(Math.hypot(position.x-r.x,position.z-r.z)<7)r.hero.face?.(position.x-r.x,position.z-r.z,dt);}}}for(const d of decorations){if(d.orb){d.orb.rotation.y=time*.2;d.orb.position.y=3.5+Math.sin(time)*.15;}}if(premiumPlatform)premiumPlatform.tick(time);},dispose(){regional.dispose();paris.dispose();heritage?.dispose();flora.dispose();authored.dispose();residents.forEach(r=>r.hero.dispose());architecture.dispose();premiumWater.disposeReflection?.();owned.forEach(r=>r.dispose());}};
}

...
export async function loadWorldModels(){
 ...
 const [hero,nexus,collisionResponse,atlas]=await Promise.all([
 ...
 const assets=[hero.scene,nexus.scene];
 ...
 return {hero,nexus,collisions,atlas,dispose(){
...
export function createKais(asset){
 ...
 return {object,
  ...
  dispose(){mixer.stopAllAction();mixer.uncacheRoot(object);object.traverse(o=>{if(o.isSkinnedMesh)o.skeleton.dispose();});}
 };
}

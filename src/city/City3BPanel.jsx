...
export function City3BPanel({uid,onLogin,onNotice,onWorldCitySync}){
 ...
 const refresh=useCallback(async()=>{if(!uid)return;setLoading(true);setError('');try{const next=await city3bRequest('snapshot',{},uid);if(mounted.current&&scope.current===uid){setData(next);if(next?.city)onWorldCitySync?.();}}catch(e){if(mounted.current&&scope.current===uid)setError(e.message);}finally{if(mounted.current&&scope.current===uid)setLoading(false);}},[uid,onWorldCitySync]);
 useEffect(()=>{setData(null);setBusy('');refresh();},[refresh]);
 async function action(kind,body={}){if(!uid||busy)return;setBusy(kind);setError('');try{const next=await city3bRequest(kind,body,uid);if(!mounted.current||scope.current!==uid)return;setData(next);onNotice?.(kind==='sync_world'?'Ville 3B synchronisée avec le Monde du 3B.':'Ville 3B mise à jour.');if(next?.city)await onWorldCitySync?.();}catch(e){if(mounted.current&&scope.current===uid)setError(e.message);}finally{if(mounted.current&&scope.current===uid)setBusy('');}}

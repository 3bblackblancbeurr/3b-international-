...
export function ArenaStage({state,side=0,cardId,avatar}){
 const ref=useRef(null),liveState=useRef({state,side,cardId,avatar}),[error,setError]=useState('');liveState.current={state,side,cardId,avatar};
 useEffect(()=>{
  ...
  function tick(now){if(disposed)return;raf=requestAnimationFrame(tick);const dt=Math.min(.05,(now-at)/1000);at=now;if(document.hidden)return;elapsed+=dt;impact+=dt;
   ...
   for(const [i,a] of actors.entries()){a.update(dt);a.object.rotation.y=solo?rot:i===s.side?.7:-.7;if(!solo){const x=i===s.side?-1.45:1.45;a.object.position.x=x+(i===who&&!reduced?Math.sin(Math.min(1,impact/.65)*Math.PI)*.5*(x<0?1:-1):0);}}
   const intro=!solo&&!reduced?Math.max(0,1-elapsed/2.5):0,zoom=solo?4.5:5.2+intro*1.5,orbit=solo?0:rot*.2;camera.position.set(Math.sin(orbit)*zoom,solo?2.1:2.15+intro*.5,Math.cos(orbit)*zoom);camera.lookAt(0,.95,0);renderer.render(scene,camera);
  }

...
export function ArenaStage({state,side=0,cardId,avatar,focus='body',pose='idle',angle=null,cinematic=null}){
 ...
 useEffect(()=>{
  ...
  const stageProps=[];
  const mesh=(g,m,parent=scene)=>{geometry.push(g);const o=new THREE.Mesh(g,m);o.receiveShadow=true;parent.add(o);if(parent===scene)stageProps.push(o);return o;};
  ...
  let assetEpoch=0,lastAngle=null;let lastFilm='',lastRound=null,lastWinner=null,introAge=0,resultAge=0;
  ...
  function tick(now){if(disposed)return;raf=requestAnimationFrame(tick);const rawDt=Math.min(.05,Math.max(0,(now-at)/1000));at=now;if(document.hidden)return;
   ...
   scene.background=s.avatar&&!film?null:backdrop;for(const prop of stageProps)prop.visible=!s.avatar||!!film;
   if(next.join('|')!==ids.join('|')){const epoch=++assetEpoch;actors.forEach(a=>{scene.remove(a.object);a.dispose();});ids=next;actors=next.map((id,index)=>{const a=createLivingActor(library,{...(s.avatar?{avatar:s.avatar}:{card:id}),onError:message=>{if(!disposed&&epoch===assetEpoch)setError(message);},onLoad:()=>{if(!disposed&&epoch===assetEpoch)setError('');}});a.object.position.set(solo?0:index===s.side?-1.45:1.45,0,0);a.object.rotation.y=solo?0:index===s.side?.65:-.65;scene.add(a.object);return a;});lastFilm='';}
   if(s.state){
   ...
   if(film){
    ...
    const fovBlend=reduced?1:1-Math.exp(-dt*4.5);camera.fov+=((shot.fov||38)-camera.fov)*fovBlend;camera.updateProjectionMatrix();

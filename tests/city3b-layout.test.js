...
import {CITY_DISTRICTS,boxesOverlap,districtPosition,isPlacementValid,landHalfSize,placementBox,rotatedFootprint} from '../src/city/city3b-layout.js';
...
test('rotation swaps rectangular footprints but preserves square footprints',()=>{assert.deepEqual(rotatedFootprint({footprint:{w:4,h:2}},0),{w:4,h:2});assert.deepEqual(rotatedFootprint({footprint:{w:4,h:2}},90),{w:2,h:4});assert.deepEqual(rotatedFootprint({footprint:{w:3,h:3}},270),{w:3,h:3})});

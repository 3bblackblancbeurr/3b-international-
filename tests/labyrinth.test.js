...
test('the library starts with online-only Penalty Rush and keeps the original key race',()=>{
 assert.deepEqual(GAME_CATALOG.map(g=>g.id),['penalty-rush','arena','tower','maze','key-race']);assert.equal(GAME_CATALOG[4].href,KEY_RACE_URL);
...
test('light pulse has local range, an energy cost and a cooldown; its trail follows actual passages',()=>{
 ...
 const near=new Maze(2);placeMazePlayer(near,{x:25,y:16});near.action();assert.ok(near.shadowStun>0);assert.ok(paths(near.grid,near.cell).dist[near.shadow.y*near.cols+near.shadow.x]>=4);
});
test('the shadow pursues only on sight or sound, searches a last known position, then patrols',()=>{
 const g=new Maze(2);g.time=11;placeMazePlayer(g,{x:25,y:17});g.updateShadow(.01);assert.equal(g.shadowMode,'hunt');assert.deepEqual(g.lastKnown,g.cell);
 placeMazePlayer(g,{x:1,y:17});g.moving=false;g.shadowClock=0;g.updateShadow(.01);assert.equal(g.shadowMode,'search');assert.deepEqual(g.lastKnown,{x:25,y:17});
...
test('the sanctuary is safe, a hit gives a real escape window, and three unrecovered hits are fatal',()=>{
 ...
 placeMazePlayer(g,{x:25,y:15});g.shadow={...g.cell};g.update(.01);assert.equal(g.player.hp,66);assert.equal(g.status,'playing');
...
test('seals replenish life and light once and all three are required at the portal',()=>{
 const g=new Maze(3);placeMazePlayer(g,{...g.exit});g.update(.01);assert.equal(g.status,'playing');
 g.player.hp=32;g.lamp=50;placeMazePlayer(g,{x:g.fragments[0].x,y:g.fragments[0].y});g.update(.01);assert.equal(g.player.hp,66);assert.ok(g.lamp>79);assert.equal(g.collected,1);
...
test('switches open their marked wall exactly once, even while the pulse is recharging',()=>{
 const g=new Maze(31),sw=g.switches[0];placeMazePlayer(g,{x:sw.x,y:sw.y});g.flashCooldown=7;const lamp=g.lamp;

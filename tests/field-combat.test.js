...
test('final circle damage cannot cross a Guardian threshold until that phase mechanic is mastered',()=>{
 ...
 assert.equal(e.enemy,finalCircleLockedEnemyFloor(e,{index:1}));
 assert.equal(finalCirclePhase(e).index,2,'a mastered hit may enter only the immediate next phase');
});

...
test('equal power sends moved attackers back to their origin',()=>{
 ...
 const out=resolveTurn(g,{ai:false,penalty:false});assert.equal(out.units.find(u=>u.id===attacker.id).sectorId,'front0');assert.ok(out.lastResolution.events.some(e=>e.type==='stalemate'));
});
test('Mega missile costs 100 Power, clears combat pieces and is consumed after one strike',()=>{
 ...
 queueMegaMissile(g,0,target);const out=resolveTurn(g,{ai:false,penalty:false});assert.equal(out.nations[0].power,0);assert.equal(unitsIn(out,target).filter(u=>u.type!=='flag').length,0);assert.ok(out.lastResolution.events.some(e=>e.type==='mega'));
});
test('infantry capture of the final enemy flag ends the campaign',()=>{
 ...
 const out=resolveTurn(g,{ai:false,penalty:false});assert.equal(out.nations[1].active,false);assert.equal(out.nations[0].flagsCaptured,1);assert.equal(out.winner,0);
});
...
test('Power is awarded separately from captures and is capped at three occupied opponent countries per round',()=>{
 ...
 const out=resolveTurn(g,{ai:false,penalty:false});assert.equal(out.nations[0].power,3);assert.ok(out.lastResolution.events.some(e=>e.type==='power'));
});

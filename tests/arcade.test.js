...
test('maze: 200 seeded maps preserve access to all objectives and opening walls never disconnects them',()=>{
 let previous='';for(let seed=1;seed<=200;seed++){const g=new Maze(seed);const now=JSON.stringify(g.grid);assert.notEqual(now,previous);previous=now;const objectives=[g.exit,...g.fragments,...g.switches,...g.lamps];let route=paths(g.grid,g.cell);for(const p of objectives)assert.ok(route.dist[p.y*g.cols+p.x]>=0);for(const sw of g.switches){placeMazePlayer(g,{x:sw.x,y:sw.y});g.action();}route=paths(g.grid,{x:1,y:1});for(const p of objectives)assert.ok(route.dist[p.y*g.cols+p.x]>=0);assert.equal(g.switches.filter(s=>s.used).length,2);}
});
test('maze: walls block movement, a fragment cannot be counted twice, and all three unlock victory',()=>{
 const g=new Maze(8);placeMazePlayer(g,{x:1,y:1});g.update(.14,{x:-1,y:0});assert.equal(g.cell.x,1);g.shadow={x:23,y:15};g.flash=30;for(const f of g.fragments){placeMazePlayer(g,{x:f.x,y:f.y});g.update(.02);g.update(.02);}assert.equal(g.collected,3);placeMazePlayer(g,{...g.exit});g.update(.02);assert.equal(g.won,true);
});
test('maze: depleted lamp and fatal damage outside the sanctuary end the run',()=>{
 const a=new Maze(2);a.lamp=.001;a.update(.02);assert.equal(a.won,false);const b=new Maze(2);placeMazePlayer(b,{x:25,y:15});b.shadow={...b.cell};b.player.hp=1;b.update(.02);assert.equal(b.won,false);
});

...
test('guard intent survives attack spam and executes as soon as recovery ends',()=>{const q=createCombatInput();q.queue('light',1);assert.equal(q.queue('guard',1.05),true);assert.equal(q.queue('heavy',1.08),false);assert.equal(q.take(1.2,true),null);assert.equal(q.take(1.3,false),'guard');assert.equal(q.take(1.31,false),null);});

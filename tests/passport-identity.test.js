...
test('city origin is displayed as Passport-controlled and not user-selectable',()=>{
 const source=readFileSync(new URL('../src/components/City3BPortal.jsx',import.meta.url),'utf8');
 const worldPanel=readFileSync(new URL('../src/city/City3BPanel.jsx',import.meta.url),'utf8');
 assert.match(worldPanel,/readOnly/);
 assert.doesNotMatch(worldPanel,/setCountry/);
 const route=readFileSync(new URL('../src/components/PassportNexus.jsx',import.meta.url),'utf8');
 assert.match(route,/NexusCityGateway/);
